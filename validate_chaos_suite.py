...
async def test_health_endpoints():
    ...
    proc = subprocess.Popen(
        ["python3", "src/main.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def test_resilience_validator():
    ...
    try:
        ...
        from deploy.resilience_validator import ResilienceValidator

        validator = ResilienceValidator(
            target_url="http://localhost:5001", config_path="deploy/chaos_scenarios.yml"
        )
        ...
        logger.info(
            f"   Blocking MTTR: {validator.slo_severity.blocking.mttr_seconds}s"
        )

        # Test validation with mock data
        validator.metrics.mttr = 50.0  # Between warning (45s) and blocking (60s)
        validator.metrics.error_rate_percent = (
            10.0  # Between warning (8%) and blocking (15%)
        )
        validator.metrics.availability_percent = (
            90.0  # Between warning (92%) and blocking (85%)
        )
...
def test_chaos_scripts():
    ...
    for script in scripts:
        ...
        if script.endswith(".sh") and not Path(script).stat().st_mode & 0o111:
...
async def main():
    ...
    if passed == total:
        logger.info(
            "🎉 All validation tests passed! Chaos suite is ready for deployment."
        )
        return 0
    else:
        logger.error(
            "💥 Some validation tests failed. Please fix issues before deployment."
        )
        return 1
