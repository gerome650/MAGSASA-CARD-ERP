...
class ProgressiveRollout:
    ...
    async def run_stage_load_test(
        self, stage: dict, service_url: str
    ) -> tuple[bool, dict]:
...
"""
    ...
    async def execute_progressive_rollout(self) -> bool:
        ...
        self.logger.info(
            f"Load testing: {'Enabled' if self.load_test_enabled else 'Disabled'}"
        )
        ...
        for i, stage in enumerate(self.rollout_stages):
            ...
            if not traffic_updated:
                if self.auto_rollback:
                    await self.rollback_deployment(
                        f"Failed to update traffic split for {stage_name}"
                    )
                return False
            ...
            if self.load_test_enabled:
                load_test_passed, load_results = await self.run_stage_load_test(
                    stage, service_url
                )
                ...
                if not load_test_passed:
                    if self.auto_rollback:
                        await self.rollback_deployment(
                            f"Load test failed for {stage_name}"
                        )
                    return False
            ...
            if not health_ok:
                if self.auto_rollback:
                    await self.rollback_deployment(
                        f"Health monitoring failed for {stage_name}"
                    )
                return False
...
"""
        ...
        for result in results:
            ...
            load_test_status = (
                "✅ PASSED" if result.get("load_test_passed", True) else "❌ FAILED"
            )
...
async def main():
    ...
    parser.add_argument(
        "--namespace", default="production", help="Kubernetes namespace"
    )
    parser.add_argument(
        "--load-test", action="store_true", help="Enable load testing at each stage"
    )
    parser.add_argument(
    ...
    parser.add_argument("--stages", type=str, help="Custom rollout stages as JSON")
