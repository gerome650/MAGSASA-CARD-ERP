            ...
            if result.returncode != 0:
                ...
                if self.fix_issues:
                    ...
                    fix_result = subprocess.run(
                        [sys.executable, 'scripts/chaos_dependency_sentinel.py', '--fix'],
                        capture_output=True,
                        text=True,
                        timeout=60
                    )
        ...
        for health_file in health_files:
            ...
            try:
                with open(health_file, 'r') as f:
                    content = f.read()
                    if '/api/health' in content or '@app.route' in content and 'health' in content:
...
def main():
    ...
    checks = [(name, func) for name, func in checks if not any(skip in name.lower() for skip in skip_list)]
