...
from utils.notion_client import NotionClient, validate_database_access
from scripts.validate_notion_schema import SchemaValidator, EXPECTED_SCHEMAS
...
class NotionSanityChecker:
    ...
    def _add_result(self, name: str, status: str, message: str, details: Optional[Dict] = None, duration: float = 0.0):
        """Add a check result."""
    ...
    def check_api_connectivity(self) -> bool:
        ...
        try:
            ...
            # Test basic API call
            search_results = client.search(query="", filter_params={"value": "database", "property": "object"})
            
            self._add_result(
                "API Connectivity",
                "success",
                f"Connected successfully in {duration:.2f}s",
                {"accessible_databases": len(search_results), "api_key_prefix": client.api_key[:20] + "..."}
            )
            return True
        ...
        for env_var, db_name in databases:
            ...
            try:
                ...
                self._add_result(
                    ...
                    {"title": title, "properties_count": len(properties), "db_id": db_id[:8] + "..."}
                )
        ...
        try:
            ...
            try:
                ...
                roadmap_result = sync_roadmap(
                    client,
                    os.getenv("MAGSASA_ROADMAP_DB_ID"),
                    dry_run=True
                )
            ...
            try:
                ...
                milestones_result = sync_milestones(
                    client,
                    os.getenv("AI_STUDIO_MILESTONES_DB_ID"),
                    dry_run=True
                )
                sync_results["Milestones"] = milestones_result.get("records_synced", 0)
                print(f"      ✅ {sync_results['Milestones']} strategic milestones verified")
            except Exception as e:
                print(f"      ❌ Milestones sync simulation failed: {e}")
                sync_results["Milestones"] = f"Error: {str(e)}"
            
            # Check if any syncs had errors
            has_errors = any(isinstance(v, str) and v.startswith("Error:") for v in sync_results.values())
            ...
            if has_errors:
                ...
                total_records = sum(v for v in sync_results.values() if isinstance(v, int))
                self._add_result(
                    "Dry-run Sync Simulation",
                    "success",
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        print(f"📈 Checks: {summary['successful']} passed, {summary['failed']} failed, {summary['warnings']} warnings")
        print()
...
Examples:
  ...
  python scripts/sanity_check_notion.py --validate-only
    ...
    parser.add_argument("--dry-run", action="store_true", 
                       help="Include dry-run sync simulation")
    parser.add_argument("--validate-only", action="store_true",
                       help="Run only schema validation")
