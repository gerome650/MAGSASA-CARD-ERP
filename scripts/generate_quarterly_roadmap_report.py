...
class QuarterlyReportGenerator:
    ...
    def generate_quarterly_report(self, quarter: str) -> str:
        ...
        report = self._build_quarterly_report(quarter, metrics, insights, recommendations)
    ...
    def _calculate_quarterly_metrics(self, milestones: List[Dict], quarter: str) -> QuarterlyMetrics:
        ...
        avg_pass_rate = (pass_rate_sum / total_milestones) if total_milestones > 0 else 0
    ...
    def _generate_quarterly_insights(self, metrics: QuarterlyMetrics) -> List[str]:
        ...
        completion_rate = (metrics.completed_milestones / metrics.total_milestones * 100) if metrics.total_milestones > 0 else 0
        if completion_rate >= 80:
            insights.append(f"🎉 Excellent milestone completion rate: {completion_rate:.0f}% ({metrics.completed_milestones}/{metrics.total_milestones})")
        elif completion_rate >= 60:
            insights.append(f"📈 Good milestone completion rate: {completion_rate:.0f}% ({metrics.completed_milestones}/{metrics.total_milestones})")
        else:
            insights.append(f"⚠️ Milestone completion rate needs improvement: {completion_rate:.0f}% ({metrics.completed_milestones}/{metrics.total_milestones})")
        
        # Progress insight
        if metrics.avg_progress >= 80:
            insights.append(f"🚀 Strong average progress: {metrics.avg_progress:.0f}% across all milestones")
        elif metrics.avg_progress >= 60:
            insights.append(f"📊 Moderate average progress: {metrics.avg_progress:.0f}% - room for acceleration")
        else:
            insights.append(f"🔄 Average progress below target: {metrics.avg_progress:.0f}% - focus on execution")
        
        # CI health insight
        if metrics.avg_workflow_pass_rate >= 90 and metrics.avg_auto_fix_success >= 80:
            insights.append(f"✅ Excellent CI health: {metrics.avg_workflow_pass_rate:.0f}% pass rate, {metrics.avg_auto_fix_success:.0f}% auto-fix success")
        elif metrics.avg_workflow_pass_rate >= 80 and metrics.avg_auto_fix_success >= 70:
            insights.append(f"📈 Good CI health: {metrics.avg_workflow_pass_rate:.0f}% pass rate, {metrics.avg_auto_fix_success:.0f}% auto-fix success")
        else:
            insights.append(f"⚠️ CI health needs attention: {metrics.avg_workflow_pass_rate:.0f}% pass rate, {metrics.avg_auto_fix_success:.0f}% auto-fix success")
        
        # Trend insight
        if metrics.improving_trends > metrics.degrading_trends:
            insights.append(f"📈 Positive trend momentum: {metrics.improving_trends} improving vs {metrics.degrading_trends} degrading")
        elif metrics.degrading_trends > metrics.improving_trends:
            insights.append(f"📉 Trend concerns: {metrics.degrading_trends} degrading vs {metrics.improving_trends} improving")
        else:
            insights.append(f"📊 Stable trends: {metrics.stable_trends} stable milestones")
        
        # Blocked milestones insight
        if metrics.blocked_milestones > 0:
            insights.append(f"🚧 {metrics.blocked_milestones} milestone(s) blocked - investigate and resolve blockers")
        
        return insights
    
    def _generate_quarterly_recommendations(self, metrics: QuarterlyMetrics) -> List[str]:
        """Generate recommendations based on quarterly metrics."""
        recommendations = []
        
        # Completion recommendations
        if metrics.completed_milestones / metrics.total_milestones < 0.6:
            recommendations.append("🎯 **Focus on Execution:** Increase milestone completion rate through better planning and resource allocation")
        
        # Progress recommendations
        if metrics.avg_progress < 60:
            recommendations.append("⚡ **Accelerate Progress:** Implement agile practices and regular check-ins to boost milestone progress")
        
        # CI health recommendations
        if metrics.avg_workflow_pass_rate < 80:
            recommendations.append("🔧 **Improve CI Stability:** Focus on reducing flaky tests and improving test reliability")
        
        if metrics.avg_auto_fix_success < 70:
            recommendations.append("🤖 **Enhance Auto-Fix:** Improve auto-fix strategies and expand coverage for common failure patterns")
        
        if metrics.avg_mttr > 10:
            recommendations.append("⏱️ **Reduce MTTR:** Implement faster debugging tools and better error reporting to decrease resolution time")
        
        # Trend recommendations
        if metrics.degrading_trends > metrics.improving_trends:
            recommendations.append("📊 **Address Degrading Trends:** Investigate root causes of declining metrics and implement corrective actions")
        
        # Blocked milestone recommendations
        if metrics.blocked_milestones > 0:
            recommendations.append("🚧 **Resolve Blockers:** Conduct blocker review sessions and implement escalation procedures")
        
        # General recommendations
        recommendations.append("📈 **Continue Monitoring:** Maintain regular milestone reviews and CI health monitoring")
        recommendations.append("🔄 **Iterate and Improve:** Use quarterly insights to refine processes and planning for next quarter")
        
        return recommendations
    
    def _build_quarterly_report(self, quarter: str, metrics: QuarterlyMetrics, 
                               insights: List[str], recommendations: List[str]) -> str:
        ...
        report_lines.append(f"- **Completed:** {metrics.completed_milestones} ({metrics.completed_milestones/metrics.total_milestones*100:.0f}%)")
        ...
        report_lines.append(f"- **Average CI Pass Rate:** {metrics.avg_workflow_pass_rate:.0f}%")
        report_lines.append(f"- **Average Auto-Fix Success:** {metrics.avg_auto_fix_success:.0f}%")
        ...
        report_lines.append("| Milestone | Status | Progress | Pass Rate | Auto-Fix | MTTR | Trend |")
        report_lines.append("|-----------|--------|----------|-----------|----------|------|-------|")
        ...
        # Footer
        report_lines.append("---\n")
        report_lines.append(f"*Quarterly report generated by CI Intelligence Agent v2.0 on {datetime.now().isoformat()}*\n")
...
Examples:
  ...
  python scripts/generate_quarterly_roadmap_report.py --quarter Q4-2024 --output reports/Q4-2024_ROADMAP_REPORT.md
    ...
    parser.add_argument(
        "--quarter",
        help="Quarter to generate report for (e.g., Q4-2024)"
    )
    parser.add_argument(
        "--auto-quarter",
        action="store_true",
    ...
    if not notion_api_key:
        print("❌ Error: Notion API key required. Set NOTION_API_KEY env var or use --notion-api-key")
        sys.exit(1)
    
    if not roadmap_db_id:
        print("❌ Error: Roadmap database ID required. Set NOTION_ROADMAP_DB_ID env var or use --roadmap-db-id")
        sys.exit(1)
