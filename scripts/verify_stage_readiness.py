...
class StageReadinessVerifier:
    ...
    def check_git_status(self) -> Dict[str, Any]:
        ...
        if exit_code == 0:
            ...
            if branch.strip() not in ['main', 'develop'] and not branch.strip().startswith('feature/'):
                check_result['issues'].append(f"Not on expected branch (current: {branch.strip()})")
        ...
        if exit_code == 0 and stdout.strip():
            ...
            check_result['issues'].append("No observability-related commits found in recent history")
    ...
    def check_dependencies(self) -> Dict[str, Any]:
        ...
        obs_req_file = self.repo_root / 'observability' / 'observability_requirements.txt'
        if obs_req_file.exists():
            with open(obs_req_file, 'r') as f:
                obs_content = f.read()
    ...
    def check_tests(self) -> Dict[str, Any]:
        ...
        exit_code, stdout, _ = self.run_command("python -m pytest --cov=observability --cov-report=term-missing -q")
        if exit_code == 0:
    ...
    def check_ci_workflows(self) -> Dict[str, Any]:
        ...
        for workflow_file in workflows_dir.glob('*.yml'):
            ...
            try:
                ...
                check_result['passed'] = False
                check_result['workflow_issues'].append(f"Invalid YAML in {workflow_file.name}: {e}")
        ...
        if obs_workflow.exists():
            with open(obs_workflow, 'r') as f:
                content = f.read()
                
            if 'validate_alert_rules.py' in content:
                self.success("observability.yml references validate_alert_rules.py")
            else:
                self.warning("observability.yml doesn't reference validate_alert_rules.py")
                
            if 'check_observability_hooks.py' in content:
                self.success("observability.yml references check_observability_hooks.py")
            else:
                self.warning("observability.yml doesn't reference check_observability_hooks.py")
    ...
    def generate_summary_report(self) -> None:
        ...
        print(f"{Colors.YELLOW}⚠️ Warnings: {self.results['summary']['warnings']}{Colors.END}")
        print(f"{Colors.RED}❌ Failures: {self.results['summary']['failures']}{Colors.END}")
    ...
    def run_verification(self) -> bool:
        ...
        self.log("Starting Stage 6.7 → 6.8.1 Readiness Audit", Colors.BOLD + Colors.CYAN, "🧩")
...
def main():
    ...
    parser.add_argument(
        '--json-output',
        type=str,
        help='Write results to JSON file for dashboards'
    )
    ...
    if args.json_output:
        verifier.save_json_report(args.json_output)
