...
def calculate_project_kpis() -> dict[str, Any]:
    ...
    variance_pct = (
        (completed_milestones - planned_completion) / planned_completion * 100
    )
    ...
    days_to_next = (
        datetime.fromisoformat(next_milestone_date) - datetime.utcnow()
    ).days
    ...
    existing_records = notion_client.query_database(
        ...
        filter_params={
            "and": [
                ...
                {"property": "Type", "select": {"equals": "KPI Summary"}},
            ]
        },
    )
...
Auto-Fix Success: {kpis['auto_fix_success']}%
    ...
    with contextlib.suppress(Exception):
        properties["Overall Completion"] = create_number_property(
            kpis["completion_pct"]
        )
    ...
    if dry_run:
        ...
        print(
            f"  Next: {kpis['next_milestone']} in {kpis['days_to_next_milestone']} days"
        )
