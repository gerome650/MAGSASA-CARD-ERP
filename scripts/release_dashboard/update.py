...
<!-- CI_SNAPSHOT_END -->"""
        ...
        for run in runs[:10]:  # Limit to 10 most recent
            ...
            status_emoji = GitHubWorkflowFetcher.get_status_emoji(
                run.get("conclusion"), run.get("status", "")
            )
            status_text = run.get("conclusion") or run.get("status", "unknown")

            duration = run.get("duration_seconds")
            duration_str = (
                GitHubWorkflowFetcher().format_duration(duration) if duration else "N/A"
            )
        ...
        if health and health.get("failing_workflows"):
            lines.extend(["", "### ⚠️ Failing Workflows (Recent)", ""])

            for fw in health["failing_workflows"][:3]:  # Top 3
                lines.append(
                    f"- **{fw['name']}**: {fw['count']} failures - [View]({fw['url']})"
                )

        lines.extend(["", "<!-- CI_SNAPSHOT_END -->"])
    ...
    def generate_readiness_score_section(self, score_data: dict[str, Any]) -> str:
        ...
        if total_score < 95:
            ...
            if score_data["core_passing"] < score_data["core_total"]:
                ...
                lines.append(
                    f"1. Complete {remaining} remaining core gate(s) - **CRITICAL**"
                )

            if score_data["deployment_passing"] < score_data["deployment_total"]:
                remaining = (
                    score_data["deployment_total"] - score_data["deployment_passing"]
                )
