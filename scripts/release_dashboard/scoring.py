...
class ReadinessScorer:
    ...
    def calculate_score(self, ci_health: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        optional_passing, optional_total = self._count_gates_from_table(optional_pattern)
        
        # Parse deployment automation
        deployment_pattern = r'## 📦 Deployment & Release Automation.*?(?=\n##|\Z)'
        deployment_passing, deployment_total = self._count_gates_from_table(deployment_pattern)
        ...
        optional_score = (optional_passing / optional_total * 100) if optional_total > 0 else 0
        deployment_score = (deployment_passing / deployment_total * 100) if deployment_total > 0 else 0
        signoff_score = (signoff_passing / signoff_total * 100) if signoff_total > 0 else 0
        ...
        if deployment_passing < deployment_total - 1:  # Allow 1 pending
            blockers.append(f"Deployment: {deployment_total - deployment_passing} items pending")
        if signoff_passing < signoff_total - 2:  # Allow 2 pending
            blockers.append(f"Sign-off: {signoff_total - signoff_passing} items pending")
    ...
    def get_score_trend(self, history_file: str = ".cache/readiness.json") -> Optional[Dict[str, Any]]:
        """
