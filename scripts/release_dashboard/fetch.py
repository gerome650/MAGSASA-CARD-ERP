...
class GitHubWorkflowFetcher:
    ...
    def __init__(self, token: Optional[str] = None, repo_full_name: Optional[str] = None, verbose: bool = False):
        """
        ...
        if not self.token:
            raise ValueError("GitHub token required. Set GH_TOKEN env var or pass token parameter")
        ...
        try:
            ...
            if e.status == 401:
                ...
                raise ValueError(f"Repository '{repo_full_name}' not found or no access.")
            else:
    ...
    def _detect_repo_from_git(self) -> str:
        ...
        try:
            ...
            raise ValueError("Failed to detect repository. Not in a git repository or no remote configured.")
    
    def get_workflow_runs(self, limit: int = 10, branch: Optional[str] = None) -> List[Dict[str, Any]]:
        """
                ...
                try:
                    ...
                    for run in runs:
                        ...
                        all_runs.append({
                            ...
                            'workflow_file': workflow.path.split('/')[-1] if workflow.path else workflow.name,
                            'display_name': self.WORKFLOW_DISPLAY_NAMES.get(
                                workflow.path.split('/')[-1] if workflow.path else workflow.name,
                                workflow.name
                            ),
                            'status': run.status,
                    ...
                    if self.verbose:
                        print(f"⚠ Warning: Could not fetch runs for workflow {workflow.name}: {e}")
                    continue
    ...
    @staticmethod
    def get_status_emoji(conclusion: Optional[str], status: str) -> str:
