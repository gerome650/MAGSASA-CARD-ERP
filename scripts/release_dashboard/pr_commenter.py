...
class PRCommenter:
    ...
    def __init__(
        ...
        try:
            ...
            if e.status == 401:
                ...
                raise ValueError(f"Repository '{owner}/{repo}' or PR #{pr_number} not found or no access.")
            else:
    ...
    def format_failing_workflows(self, failing_workflows: List[Dict[str, Any]], max_items: int = 3) -> str:
        """
...
{self.COMMENT_MARKER}"""
    ...
    def find_existing_comment(self) -> Optional[Any]:
        """
        ...
        try:
            ...
            for comment in comments:
                if self.COMMENT_MARKER in comment.body:
                    if self.verbose:
                        print(f"✓ Found existing release dashboard comment (ID: {comment.id})")
                    return comment
    ...
    def post_readiness_comment(
        ...
        """
        try:
            body = self.generate_comment_body(score_data, failing_workflows, dashboard_branch)
...
def get_repo_and_pr(
    ...
    """
    commenter = PRCommenter(token=token, owner=owner, repo=repo, pr_number=pr_number, verbose=True)
