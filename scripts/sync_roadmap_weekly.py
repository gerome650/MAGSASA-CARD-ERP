...
def sync_roadmap(notion_client: NotionClient, database_id: str, dry_run: bool = False) -> Dict[str, Any]:
    """
    ...
    for milestone in milestones:
        try:
            ...
            if metrics["drift_days"] > 0:
                update_props["Drift (days)"] = create_number_property(metrics["drift_days"])
            
            if dry_run:
                milestone_name = extract_title_value(milestone["properties"].get("Name", {}))
    ...
    completion_rate = (completed_count / total_milestones * 100) if total_milestones > 0 else 0
...
def main():
    ...
    parser.add_argument("--db-id", help="Database ID (or use MAGSASA_ROADMAP_DB_ID env var)")
    args = parser.parse_args()
    
    # Get database ID
    database_id = args.db_id or os.getenv("MAGSASA_ROADMAP_DB_ID")
    if not database_id:
        print("❌ Error: Database ID required (--db-id or MAGSASA_ROADMAP_DB_ID env var)")
        sys.exit(1)
    ...
    try:
        ...
        print(f"✅ Roadmap sync complete:")
        print(f"   Milestones Updated: {result['records_synced']}/{result['total_milestones']}")
