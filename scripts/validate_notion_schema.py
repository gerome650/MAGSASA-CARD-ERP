            ...
            for prop_schema in expected_props:
                ...
                if prop_name not in actual_properties:
                    if required:
                        ...
                        print(
                            f"  ❌ Missing required property: '{prop_name}' (type: {expected_type})"
                        )
                    else:
                        self.warnings.append(
                            f"⚠️ {db_name}: Missing optional property '{prop_name}'"
                        )
                    ...
                    if actual_type != expected_type:
                        ...
                        print(
                            f"  ❌ Type mismatch for '{prop_name}': expected {expected_type}, got {actual_type}"
                        )
                    else:
            ...
            if missing_props or type_mismatches:
                ...
                if type_mismatches:
                    for prop, expected, actual in type_mismatches:
                        self.errors.append(
                            f"   Type mismatch: {prop} (expected {expected}, got {actual})"
                        )

                print(f"\n  ❌ Validation failed for {db_name}")
                return False
            else:
                print("  ✅ All required properties present and correct")
                return True
