...
class NotionSyncAgent:
    ...
    def sync_ci_report_to_notion(self, report_path: str) -> dict[str, Any]:
        ...
        if not self.ci_reports_db_id:
            raise ValueError(
                "❌ CI Reports database ID not configured. Please set MAGSASA_CI_DB_ID environment variable."
            )
        ...
        properties = {
            ...
            "Analysis Period": create_number_property(
                metadata.get("analysis_period_days", 7)
            ),
            "Total Failures": create_number_property(metadata.get("total_failures", 0)),
            "Auto-fix Success Rate": create_number_property(
                metadata.get("auto_fix_rate", 0.0)
            ),
            "Average MTTR": create_number_property(metadata.get("avg_mttr", 0.0)),
            "Top Failure Category": create_rich_text_property(
                metadata.get("top_category", "N/A")
            ),
            "Report Content": create_rich_text_property(
                report_content[:2000]
            ),  # Truncate for Notion limits
            "GitHub Link": create_url_property(metadata.get("github_link", "")),
        }

        page_id = self.notion_client.create_page(self.ci_reports_db_id, properties)[
            "id"
        ]
    ...
    def sync_roadmap_milestone(
        self, milestone_data: MilestoneData, ci_report_page_id: str | None = None
    ) -> dict[str, Any]:
        """Sync or update a roadmap milestone in Notion."""
        if not self.roadmap_db_id:
            raise ValueError(
                "❌ Roadmap database ID not configured. Please set MAGSASA_ROADMAP_DB_ID environment variable."
            )
        ...
        if existing_milestone:
            ...
            result = self.notion_client.create_page(self.roadmap_db_id, properties)
            page_id = result["id"]
            action = "created"
                ...
                status_summary.append(
                    {
                        ...
                        "progress": extract_number_value(props.get("📊 Progress", {}))
                        or 0,
                        "target_date": extract_date_value(
                            props.get("📆 Target Date", {})
                        ),
                        "workflow_pass_rate": extract_number_value(
                            props.get("🧪 Workflow Pass Rate", {})
                        )
                        or 0.0,
                        "auto_fix_success": extract_number_value(
                            props.get("🩹 Auto-Fix Success", {})
                        )
                        or 0.0,
    ...
    def _parse_report_metadata(self, report_content: str) -> dict[str, Any]:
        ...
        failures_match = re.search(
            r"Total failures analyzed:\*\* (\d+)", report_content
        )
        if failures_match:
            metadata["total_failures"] = int(failures_match.group(1))

        # Extract auto-fix rate
        fix_rate_match = re.search(
            r"Auto-fix success rate:\*\* ([\d.]+)%", report_content
        )
        if fix_rate_match:
    ...
    def _find_milestone_by_name(self, milestone_name: str) -> dict[str, Any] | None:
    ...
    def _build_milestone_properties(
        self, milestone_data: MilestoneData, ci_report_page_id: str | None = None
    ) -> dict[str, Any]:
        ...
        properties = {
            ...
            "🧪 Workflow Pass Rate": create_number_property(
                milestone_data.workflow_pass_rate
            ),
            "🩹 Auto-Fix Success": create_number_property(
                milestone_data.auto_fix_success
            ),
        ...
        if milestone_data.target_date:
            properties["📆 Target Date"] = create_date_property(
    ...
    def calculate_milestone_progress(self, milestone_name: str) -> MilestoneData:
        ...
        if fix_data and fix_data["total_attempts"] > 0:
            auto_fix_success = (
                fix_data["successful"] / fix_data["total_attempts"]
            ) * 100
        else:
        ...
        if progress >= 90:
            status = "Completed"
        elif progress >= 50 or progress > 0:
            status = "In Progress"
        else:
            status = "Planned"
...
Examples:
  ...
  python scripts/sync_ci_report_to_notion.py --sync-notion --include-roadmap --milestone "Stage 7.3" --report reports/CI_WEEKLY_INTELLIGENCE.md
    ...
    parser.add_argument(
        "--sync-notion", action="store_true", help="Sync CI report to Notion"
    )
    parser.add_argument(
        "--include-roadmap",
        action="store_true",
    ...
    parser.add_argument("--milestone", help="Milestone name for roadmap sync")
    parser.add_argument(
        "--report",
    ...
    parser.add_argument(
        "--roadmap-status", action="store_true", help="Show current roadmap status"
    )
    ...
    try:
        ...
        if args.include_roadmap:
            ...
            result = sync_agent.sync_roadmap_milestone(
                milestone_data, ci_report_page_id if args.sync_notion else None
            )

            if result["success"]:
                print(
                    f"✅ Milestone {result['action']}: {result['milestone']} ({milestone_data.progress}% complete)"
                )
            else:
