...
class ChaosWeeklyReporter:
    ...
    def __init__(self, db_path: str = "reports/chaos_history.db", lookback_days: int = 7):
    ...
    def record_chaos_run(self, status: str, total: int, successful: int, 
    ...
    def record_failure(self, run_id: int, category: str, message: str, fixed: bool = False):
    ...
    def calculate_metrics(self, runs: List[Dict], failures: List[Dict], 
                         healing_attempts: List[Dict]) -> Dict:
        ...
        if healing_attempts:
            ...
            metrics['healing_success_rate'] = (successful_healings / len(healing_attempts)) * 100
        
        return metrics
    
    def generate_markdown_report(self, output_file: str = "reports/chaos_weekly_report.md"):
...
"""
        ...
        if runs:
            report += "| Timestamp | Status | Scenarios | Success | Failed | Duration |\n"
            report += "|-----------|--------|-----------|---------|--------|----------|\n"
        ...
        if metrics['failures_by_category'].get('dependency', 0) > 0:
            report += "- **Dependency issues detected.** Run dependency sentinel regularly.\n"
        ...
        report += "\n---\n\n"
        report += "*This report is automatically generated by the Chaos Weekly Reporter*\n"
    ...
    def import_from_json(self, json_file: str) -> bool:
        ...
        try:
            ...
            if 'total_scenarios' in data:
                ...
                self.logger.info(f"Imported {len(data.get('attempts', []))} healing attempts")
