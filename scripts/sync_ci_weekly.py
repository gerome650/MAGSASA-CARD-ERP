...
def get_ci_intelligence_data() -> dict[str, Any]:
    ...
    reports_dir / "CI_WEEKLY_INTELLIGENCE.md"
...
def sync_ci_reports(
    ...
    existing_records = notion_client.query_database(
        database_id, filter_params={"property": "Week Of", "date": {"equals": week_of}}
    )

    # Format top failures as text
    top_failures_text = "\n".join(
        [f"• {failure}" for failure in ci_data["top_failures"][:5]]
    )

    # Format recommendations as text
    recommendations_text = "\n".join(
        [f"• {rec}" for rec in ci_data["recommendations"][:5]]
    )
