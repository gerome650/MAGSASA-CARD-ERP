...
class ControlCenterRebuilder:
                ...
                if prop_config["type"] == "relation":
                    ...
                    if related_db and related_db in self.created_databases:
                        ...
                        try:
                            ...
                            current_props[prop_name]["relation"]["database_id"] = (
                                self.created_databases[related_db]
                            )
                            ...
                            self.notion_client._make_request("PATCH", url, json=payload)
                            print(
                                f"✅ Updated relation {prop_name} in {db_config['title']}"
                            )
            ...
            for sample_item in sample_data:
                try:
                    ...
                    properties = self._convert_sample_data_to_properties(
                        sample_item, db_config["properties"]
                    )
    ...
    def _convert_sample_data_to_properties(
        self, sample_item: dict[str, Any], property_configs: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for page_key, page_config in pages.items():
            try:
                ...
                result = self.notion_client.create_page(
                    self.parent_page_id, page_properties
                )
                page_id = result["id"]
    ...
    def create_views(self) -> dict[str, str]:
        ...
        for view_key, view_config in views.items():
            try:
                ...
                print(
                    f"✅ View '{view_config['title']}' should be created manually in Notion:"
                )
    ...
    def rebuild_all(self):
        ...
        try:
            ...
            summary_file = (
                Path(__file__).parent.parent / "CONTROL_CENTER_REBUILD_SUMMARY.json"
            )
...
Examples:
  ...
  python scripts/rebuild_control_center.py --show-summary
    ...
    parser.add_argument(
        "--create-databases-only", action="store_true", help="Create databases only"
    )
    parser.add_argument(
        "--add-sample-data",
        action="store_true",
    ...
    parser.add_argument(
        "--create-pages-only", action="store_true", help="Create pages only"
    )
    parser.add_argument(
        "--show-summary", action="store_true", help="Show summary of created databases"
    )
    parser.add_argument(
        "--json-file",
