...
def should_ignore_file(filepath: str) -> bool:
    ...
    """
    return any(re.search(pattern, filepath) for pattern in IGNORE_PATTERNS)
...
def check_file_for_hooks(filepath: str) -> dict[str, list[str]]:
    ...
    """
    found_hooks = {"metrics": [], "tracing": [], "logging": []}
...
def print_results(
    changed_files: list[str], hooks_found: dict[str, list[str]], strict: bool = False
) -> bool:
    """
    ...
    print(
        f"   {'✅' if has_metrics else '❌'} Metrics:  {len(hooks_found['metrics'])} patterns"
    )
    print(
        f"   {'✅' if has_tracing else '❌'} Tracing:  {len(hooks_found['tracing'])} patterns"
    )
    print(
        f"   {'✅' if has_logging else '❌'} Logging:  {len(hooks_found['logging'])} patterns"
    )
    print()
...
def main():
    ...
    parser.add_argument(
        "--files", nargs="+", help="Specific files to check (instead of git diff)"
    )
    ...
    try:
        ...
        if args.files:
            changed_files = [
                f for f in args.files if f.endswith(".py") and not should_ignore_file(f)
            ]
        else:
            changed_files = get_changed_files()

        # Check for observability hooks
        hooks_found = {"metrics": [], "tracing": [], "logging": []}
