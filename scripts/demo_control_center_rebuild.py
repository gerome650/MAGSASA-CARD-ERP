...
class ControlCenterRebuildDemo:
    ...
    def demonstrate_database_creation(self):
        ...
        for db_key, db_config in databases.items():
            ...
            for prop_name, prop_config in db_config["properties"].items():
                ...
                required = "Required" if prop_config.get("required", True) else "Optional"
            ...
            if sample_count > 0:
                ...
                for i, sample in enumerate(db_config["sample_data"][:2], 1):
                    title = sample.get("Name") or sample.get("Stage Name") or sample.get("Metric Name") or f"Sample {i}"
                    print(f"      {i}. {title}")
                if sample_count > 2:
    ...
    def demonstrate_sample_data_creation(self):
        ...
        for db_key, db_config in databases.items():
            ...
            for i, sample_item in enumerate(sample_data, 1):
                title = sample_item.get("Name") or sample_item.get("Stage Name") or sample_item.get("Metric Name") or f"Sample {i}"
    ...
    def demonstrate_page_creation(self):
        ...
        for page_key, page_config in pages.items():
            ...
            for content_item in content_items[:3]:  # Show first 3 content items
                ...
                text = content_item.get("text", "")[:50] + "..." if len(content_item.get("text", "")) > 50 else content_item.get("text", "")
    ...
    def demonstrate_environment_variables(self):
        ...
        print(f"   # Copy the values above to your .env file")
        print(f"   # Then run: source .env")
        print(f"   # Test with: python scripts/test_notion_connection.py")
    ...
    def generate_demo_summary(self):
        ...
        summary = {
            ...
            "sample_data_items": sum(len(db_config.get("sample_data", [])) for db_config in self.schema.get("databases", {}).values()),
            "databases": {},
    ...
    def print_demo_summary(self, summary: Dict[str, Any]):
        ...
        print(f"👁️ VIEW OVERVIEW:")
        print("-" * 50)
        for view_key, view_info in summary["views"].items():
        ...
        print(f"🚀 NEXT STEPS TO ACTUALLY REBUILD:")
        print("-" * 50)
    ...
    def run_demo(self):
        ...
        try:
            ...
            # Save demo summary
            demo_summary_file = Path(__file__).parent.parent / "CONTROL_CENTER_DEMO_SUMMARY.json"
...
def main():
    ...
    if not json_file.exists():
        ...
        print("Please ensure MAGSASA-CARD_Control_Center.json exists in the project root.")
        sys.exit(1)
