...
class TwoWaySyncAgent:
    ...
    def _fetch_notion_milestones(self) -> list[dict[str, Any]]:
        ...
        if not self.roadmap_db_id:
            print(
                "⚠️ Roadmap database ID not configured. Skipping Notion milestone fetch."
            )
            return []
    ...
    def _fetch_github_roadmap_issues(self) -> list[dict[str, Any]]:
        ...
        params = {"labels": "roadmap,milestone", "state": "all", "per_page": 100}
    ...
    def _process_milestone_changes(
        self, milestone: dict[str, Any], dry_run: bool
    ) -> list[SyncChange]:
        ...
        try:
            ...
            if github_issue:
                ...
                if github_issue["state"] != current_github_state:
                    if not dry_run:
                        self._update_github_issue_status(
                            github_issue["number"], current_github_state
                        )
                ...
                if progress > 0:
                    ...
                    if progress_text not in github_issue["body"]:
                        if not dry_run:
                            self._update_github_issue_progress(
                                github_issue["number"], progress
                            )
    ...
    def _process_github_issue_changes(
        self, issue: dict[str, Any], dry_run: bool
    ) -> list[SyncChange]:
        ...
        try:
            ...
            if notion_milestone:
                ...
                if github_status != current_notion_status:
                    if not dry_run:
                        self._update_notion_milestone_status(
                            notion_milestone["id"], github_status
                        )
    ...
    def _find_github_issue_for_milestone(
    ...
    def _find_notion_milestone_by_name(
        self, milestone_name: str
    ) -> dict[str, Any] | None:
    ...
    def _map_github_state_to_notion(self, github_state: str) -> str:
        ...
        mapping = {"open": "In Progress", "closed": "Completed"}
    ...
    def _update_notion_milestone_status(self, page_id: str, status: str):
        ...
        properties = {"✅ Status": create_select_property(status)}

        self.notion_client.update_page(page_id, properties)

    def _extract_milestone_name_from_issue(self, issue: dict[str, Any]) -> str | None:
        """Extract milestone name from GitHub issue."""
        title = issue["title"]
        body = issue.get("body", "")

        # Look for milestone patterns in title
        patterns = [r"milestone[:\s]+(.+)", r"stage[:\s]+(.+)", r"roadmap[:\s]+(.+)"]
...
Examples:
  ...
  python scripts/notion_two_way_sync.py --full-sync --dry-run
    ...
    parser.add_argument(
        "--full-sync", action="store_true", help="Perform full bidirectional sync"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--github-repo", help="GitHub repository (or set GITHUB_REPOSITORY env var)"
    )
    ...
    if not github_repo:
        print(
            "❌ Error: GitHub repository required. Set GITHUB_REPOSITORY env var or use --github-repo"
        )
        sys.exit(1)

    try:
        # Initialize sync agent (now uses centralized client and environment variables)
        sync_agent = TwoWaySyncAgent(github_token=github_token, github_repo=github_repo)
        ...
        if args.sync_from_notion:
            ...
            for change in changes:
                print(
                    f"  - {change.type}: {change.milestone_name} ({change.old_value} → {change.new_value})"
                )
