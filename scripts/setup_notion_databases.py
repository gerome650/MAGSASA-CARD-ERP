...
class NotionDatabaseSetup:
    ...
    def create_database(self, title: str, description: str, 
    ...
    def show_schema_documentation(self):
        ...
        print("**Purpose:** Store weekly CI intelligence reports with metrics and analysis\n")
        print("**Properties:**\n")
        ...
        for prop_name, prop_config in roadmap_schema.items():
            ...
            if prop_type == "select":
                ...
                print(f"- **{prop_name}** ({prop_type}: links to CI Intelligence Reports)")
            else:
                print(f"- **{prop_name}** ({prop_type})")
...
Examples:
  ...
  python scripts/setup_notion_databases.py --create-databases
    ...
    parser.add_argument(
        "--create-databases",
        action="store_true",
        help="Create databases automatically"
    )
    parser.add_argument(
        "--show-schema",
        action="store_true",
    ...
    if args.create_databases:
        ...
        if not parent_page_id:
            print("❌ Error: Parent page ID required. Set NOTION_PARENT_PAGE_ID env var or use --parent-page-id")
            sys.exit(1)
        ...
        try:
            ...
            print("```bash")
            print("python scripts/sync_ci_report_to_notion.py --sync-notion --include-roadmap --milestone 'Stage 7.3'")
            print("```")
