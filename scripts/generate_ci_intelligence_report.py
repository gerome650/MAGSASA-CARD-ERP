...
class CIIntelligenceReportGenerator:
    ...
    def generate_report(self, days: int = 7) -> str:
        ...
        key_learnings = self._generate_key_learnings(
            since_date, top_failures, auto_fix_stats
        )
        recommendations = self._generate_recommendations(
            top_failures, auto_fix_stats, mttr_trends
        )
    ...
    def _generate_executive_summary(self, since_date: str, days: int) -> dict[str, Any]:
        ...
        auto_fix_rate = (
            (successful_fixes / total_attempts * 100) if total_attempts > 0 else 0
        )
    ...
    def _get_top_failure_categories(
        self, since_date: str, limit: int = 5
    ) -> list[dict[str, Any]]:
    ...
    def _generate_key_learnings(
        self, since_date: str, top_failures: list[dict], auto_fix_stats: dict
    ) -> list[str]:
        ...
        if auto_fix_stats["total_attempts"] > 0:
            success_rate = (
                auto_fix_stats["successful"] / auto_fix_stats["total_attempts"] * 100
            )
            if success_rate > 80:
                learnings.append(
    ...
    def _generate_recommendations(
        self, top_failures: list[dict], auto_fix_stats: dict, mttr_trends: dict
    ) -> list[str]:
        ...
        if auto_fix_stats["total_attempts"] > 0:
            low_success_strategies = [
                s for s in auto_fix_stats["top_strategies"] if s["success_rate"] < 50
            ]
        ...
        if mttr_trends["daily_mttr"]:
            recent_mttr = (
                mttr_trends["daily_mttr"][-1]["avg_mttr"]
                if mttr_trends["daily_mttr"]
                else 0
            )
            if recent_mttr > 5:
    ...
    def _build_markdown_report(
        ...
        fix_rate_trend = (
            "↑"
            if es["auto_fix_rate_change"] > 0
            else "↓" if es["auto_fix_rate_change"] < 0 else "→"
        )
        mttr_trend = (
            "↓" if es["mttr_change"] < 0 else "↑" if es["mttr_change"] > 0 else "→"
        )

        report_lines.append(
            f"- 🛠️ **Auto-fix success rate:** {es['auto_fix_rate']:.0f}% {fix_rate_trend}"
        )
        report_lines.append(
            f"- 📉 **MTTR:** {es['avg_mttr']:.1f} min ({mttr_trend} {abs(es['mttr_change']):.0f}%)"
        )
        report_lines.append(
            f"- 🔁 **Top recurring issue:** {es['top_recurring']['category']} – {es['top_recurring']['count']} occurrences"
        )
        report_lines.append(
            f"- 📈 **Total failures analyzed:** {es['total_failures']}\n"
        )
        ...
        for failure in top_failures:
            ...
            fix_status = (
                "✅"
                if failure["auto_fix_rate"] > 50
                else "⚠️" if failure["auto_fix_rate"] > 0 else "❌"
            )
        ...
        if auto_fix_stats["total_attempts"] > 0:
            ...
            report_lines.append(
                f"- 🔧 **{auto_fix_stats['unique_strategies']}** unique fix strategies employed"
            )
        ...
        if daily_breakdown:
            ...
            for day in daily_breakdown:
                report_lines.append(
                    f"| {day['date']} | {day['failures']} | {day['categories']} |"
                )
            report_lines.append("")
        ...
        if mttr_trends["daily_mttr"]:
            ...
            avg_mttr = sum(d["avg_mttr"] for d in mttr_trends["daily_mttr"]) / len(
                mttr_trends["daily_mttr"]
            )
            ...
            for trend in mttr_trends["daily_mttr"]:
                report_lines.append(
                    f"| {trend['date']} | {trend['avg_mttr']:.2f} | {trend['fixes']} |"
                )
            report_lines.append("")
        ...
        report_lines.append("---\n")
        report_lines.append(
            f"*Report generated by CI Intelligence Agent v2.0 on {datetime.now().isoformat()}*\n"
        )
        report_lines.append(
...
def main():
    ...
    parser.add_argument(
        "--json", action="store_true", help="Output in JSON format instead of markdown"
    )
    ...
    try:
        ...
        print(
            "ℹ️ Run CI analysis first to populate the history database", file=sys.stderr
        )
        sys.exit(1)
