...
class CIAgentCLI:
            ...
            if conclusion == "failure":
                ...
                subprocess.run(
                    [
                        ...
                        str(run_id),
                        "--json-output",
                        "latest-failure-analysis.json",
                        "--markdown-output",
                        "latest-failure-analysis.md",
                    ]
                )
    ...
    def show_stats(self):
        ...
        try:
            ...
            # Date range
            cursor.execute(
                "SELECT MIN(timestamp) as first, MAX(timestamp) as last FROM failures"
            )
            ...
            stats = [
                ...
                [
                    ...
                    (
                        f"{(successful_attempts/total_attempts*100):.1f}%"
                        if total_attempts > 0
                        else "N/A"
                    ),
                ],
                [
                    "Data Range",
                    f"{first_date[:10] if first_date else 'N/A'} to {last_date[:10] if last_date else 'N/A'}",
                ],
            ]
            ...
            print(
                tabulate(
                    top_cats, headers=["#", "Category", "Count"], tablefmt="simple"
                )
            )
    ...
    def show_recent_fixes(self, limit: int = 10):
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                time_taken = (
                    f"{row['resolution_time_minutes']:.1f}min"
                ...
                fixes.append(
                    [timestamp, category.title(), strategy, success, time_taken]
                )
    ...
    def _get_trend_arrow(self, trend: str) -> str:
        """Get arrow for trend."""
        arrows = {"rising": "🔥", "improving": "📉", "stable": "📊"}
    ...
    def sync_to_notion(self, include_roadmap: bool = False, milestone: str = None):
        ...
        try:
            ...
            cmd = [
                ...
                "--sync-notion",
                "--report",
                "reports/CI_WEEKLY_INTELLIGENCE.md",
            ]
    ...
    def show_roadmap_status(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(sync_script), "--roadmap-status"],
                capture_output=True,
                text=True,
                check=True,
            )
...
Examples:
  ...
  python scripts/ci_agent_cli.py --roadmap-status
    ...
    parser.add_argument(
        "--generate-report", action="store_true", help="Generate intelligence report"
    )
    parser.add_argument(
        "--show-trends",
        action="store_true",
    ...
    parser.add_argument(
        "--recent-fixes", action="store_true", help="Show recent fix attempts"
    )
    parser.add_argument(
        "--sync-notion", action="store_true", help="Sync CI report to Notion"
    )
    parser.add_argument(
        "--sync-roadmap", action="store_true", help="Sync roadmap milestone to Notion"
    )
    parser.add_argument(
        "--roadmap-status", action="store_true", help="Show roadmap status from Notion"
    )
