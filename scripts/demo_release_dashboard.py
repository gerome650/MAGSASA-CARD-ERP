...
def demo_readiness_calculation():
    ...
    deployment_score = (sum(deployment_automation.values()) / len(deployment_automation)) * 100
    ...
    print(f"📊 Core Gates: {sum(core_gates.values())}/{len(core_gates)} = {core_score:.1f}% (weight: 50%)")
    print(f"📊 Optional Gates: {sum(optional_gates.values())}/{len(optional_gates)} = {optional_score:.1f}% (weight: 20%)")
    print(f"📊 Deployment Automation: {sum(deployment_automation.values())}/{len(deployment_automation)} = {deployment_score:.1f}% (weight: 20%)")
    print(f"📊 Final Sign-Off: {sum(final_signoff.values())}/{len(final_signoff)} = {signoff_score:.1f}% (weight: 10%)")
...
def demo_ci_snapshot():
    ...
    for run in mock_runs:
        ...
        print(f"| {run['display_name']} | {status_emoji} {run['conclusion']} | "
