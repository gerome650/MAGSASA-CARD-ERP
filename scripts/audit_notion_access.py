...
class NotionAccessAuditor:
    ...
    def check_environment_variables(self) -> Tuple[bool, Dict[str, str]]:
        ...
        for var, description in optional_vars.items():
            ...
            if not value or not value.strip():
                ...
                env_values[var] = value
                if "API_KEY" in var or "TOKEN" in var:
                    display_value = value[:20] + "..." + value[-4:] if len(value) > 24 else "***"
                else:
                    display_value = value[:20] + "..." if len(value) > 20 else value
                print(f"✅ {var}: {display_value}")
    ...
    def test_api_connectivity(self, api_key: str) -> Tuple[bool, Optional[Any]]:
        """
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(f"❌ 403 Forbidden: Integration has no access permissions")
    ...
    def list_accessible_resources(self, headers: Dict) -> Tuple[List[Dict], List[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.warnings.append(f"Could not list databases: {response.status_code}")
    ...
    def verify_required_databases(self, headers: Dict, env_vars: Dict, accessible_dbs: List[Dict]) -> List[DatabaseInfo]:
        """
        ...
        for env_var, expected_name in required_dbs:
            ...
            if not db_id:
                ...
                database_infos.append(DatabaseInfo(
                    name=expected_name,
                    id="NOT_SET",
                    id_prefix="N/A",
                    accessible=False,
    ...
    def print_access_table(self, database_infos: List[DatabaseInfo]):
        ...
        # Print table header
        print(f"{'Database Name':<35} {'ID Prefix':<15} {'Access':<10} {'Workspace':<20}")
        print("-" * 80)
        
        # Print each database
        for db_info in database_infos:
            access_icon = "✅ Yes" if db_info.accessible else "❌ No"
            print(f"{db_info.name:<35} {db_info.id_prefix:<15} {access_icon:<10} {db_info.workspace:<20}")
    ...
    def print_remediation_steps(self):
        ...
        print("  • Verify your Notion integration at: https://www.notion.so/my-integrations")
    ...
    def generate_audit_result(self, env_vars: Dict, database_infos: List[DatabaseInfo],
                            total_dbs: int, total_pages: int) -> AuditResult:
        """
        ...
        Returns:
            AuditResult object
        """
        accessible_count = sum(1 for db in database_infos if db.accessible)
        ...
        return AuditResult(
            ...
            api_key_valid=len(self.errors) == 0 or not any("401" in e for e in self.errors),
            ...
            accessible_databases=[
                ...
                for db in database_infos if db.accessible
            ],
    ...
    def run_audit(self) -> int:
        ...
        api_ok, headers_or_error = self.test_api_connectivity(env_vars["NOTION_API_KEY"])
        if not api_ok:
        ...
        database_infos = self.verify_required_databases(headers, env_vars, accessible_dbs)
        ...
        if self.errors:
            ...
            print(f"Total accessible resources: {len(accessible_dbs)} databases, {len(accessible_pages)} pages")
            ...
            print()
            print("Next steps:")
            print("  1. Run full sanity check: python scripts/notion_cli.py sanity-check")
            print("  2. Test sync (dry-run): python scripts/notion_cli.py sync --all --dry-run")
            ...
            if self.auto_fix:
                ...
                try:
                    ...
                    print("Please run manually: python scripts/notion_cli.py sanity-check")
                    return 0
