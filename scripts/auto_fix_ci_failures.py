...
class FixStrategyManager:
    ...
    def __init__(self, db_path: str = "ci_failure_history.db"):
        ...
        self.conn = (
            sqlite3.connect(str(self.db_path)) if self.db_path.exists() else None
        )
    ...
    def _calculate_priority(
        self, confidence: float, success_rate: float, severity: str
    ) -> int:
...
class CIAutoFixer:
    ...
    def analyze_and_fix(self, analysis_file: str) -> bool:
        ...
        # Prioritize fixes based on historical success rates
        prioritized_analyses = self.strategy_manager.prioritize_fixes(
            auto_fixable_analyses
        )
    ...
    def _create_auto_fix_branch(self) -> str | None:
    ...
    def _fix_permission_issue(self, analysis: dict[str, Any]) -> bool:
        ...
        if fix_command:
            try:
                ...
                subprocess.run(fix_command.split(), cwd=self.repo_path, check=True)
    ...
    def _fix_network_timeout_issue(self, analysis: dict[str, Any]) -> bool:
        ...
        print(
            "🌐 Network timeout issue identified - manual intervention may be required"
        )
    ...
    def _generate_commit_message(self, analyses: list[dict[str, Any]]) -> str:
        ...
        categories = list({a.get("category", "unknown") for a in analyses})

        # Calculate overall success rate
        total_success_rate = 0.0
        if analyses:
            success_rates = [
                a.get("_success_rate", 0.0) for a in analyses if "_success_rate" in a
            ]
            total_success_rate = (
        ...
        for i, fix in enumerate(self.fixes_applied, 1):
            ...
            commit_message += (
                f"   - Historical Success Rate: {success_rate_info:.1%}\n\n"
            )
    ...
    def create_pull_request(self, base_branch: str = None) -> str | None:
        ...
        try:
            ...
            cmd = [
                ...
                "create",
                "--title",
                f"🔧 Auto-fix CI failures ({len(self.fixes_applied)} fixes)",
                "--body",
                pr_description,
                "--base",
                base_branch,
    ...
    def _generate_pr_description(self) -> str:
        ...
        description += "---\n"
        description += (
            "*This PR was automatically generated by the CI/CD self-healing system.*"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Auto-fix CI/CD failures with adaptive strategies"
    )
    ...
    parser.add_argument(
        "--create-pr", action="store_true", help="Create pull request after fixing"
    )
