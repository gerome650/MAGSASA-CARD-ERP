#!/usr/bin/env python3
import os, subprocess, sys, datetime, shlex, json
...
def main():
    log(f"Remediator start: ns={NS} deploy={DEPLOY} action={ACTION} scale_to={SCALE_TO}")
    # Check deployment exists
    rc, out, err = run(f"kubectl -n {shlex.quote(NS)} get deploy {shlex.quote(DEPLOY)} -o jsonpath='{{.metadata.name}}'")
    if rc != 0:
    ...
    if ACTION == "restart":
        ...
        rc, out, err = run(f"kubectl -n {shlex.quote(NS)} rollout restart deploy/{shlex.quote(DEPLOY)}")
        if rc == 0:
        ...
        rc, out, err = run(f"kubectl -n {shlex.quote(NS)} scale deploy/{shlex.quote(DEPLOY)} --replicas={shlex.quote(SCALE_TO)}")
        if rc == 0:
