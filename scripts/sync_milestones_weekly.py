...
def sync_milestones(
    ...
    for milestone_data in AI_STUDIO_MILESTONES:
        try:
            ...
            print(
                f"⚠️ Warning: Failed to sync milestone '{milestone_data['stage_name']}': {e}"
            )
            continue

    # Calculate stats
    completed_milestones = [
        m for m in AI_STUDIO_MILESTONES if m["status"] == "Completed"
    ]
    in_progress_milestones = [
        m for m in AI_STUDIO_MILESTONES if m["status"] == "In Progress"
    ]
...
def main():
    ...
    parser.add_argument(
        "--db-id", help="Database ID (or use AI_STUDIO_MILESTONES_DB_ID env var)"
    )
    ...
    if not database_id:
        print(
            "❌ Error: Database ID required (--db-id or AI_STUDIO_MILESTONES_DB_ID env var)"
        )
        sys.exit(1)
    ...
    try:
        ...
        print("✅ Milestones sync complete:")
        print(
            f"   Total Synced: {result['records_synced']}/{result['total_milestones']}"
        )
