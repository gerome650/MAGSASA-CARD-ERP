...
class ReleasePipelineVerifier:
    ...
    def check_linting(self) -> bool:
        ...
        # Check ruff
        success, output = self.run_command(["uv", "run", "ruff", "check", "src/", "packages/"])
        if not success:
            self.log(f"Ruff linting failed: {output}", "error")
            return False
        
        # Check black formatting
        success, output = self.run_command(["uv", "run", "black", "--check", "src/", "packages/"])
        if not success:
    ...
    def check_readiness_score(self) -> bool:
        ...
        try:
            ...
            success, output = self.run_command([
                "python3", "scripts/update_release_dashboard.py", 
                "--check-only", "--verbose"
            ])
                    ...
                    if 'readiness' in line.lower() and '%' in line:
                        ...
                        if match:
                            ...
                            if score >= 90:
                                ...
                                self.log(f"Readiness score: {score}% (FAIL - need >=90%)", "error")
                                return False
...
def main():
    ...
    parser.add_argument("--ci", action="store_true", help="Run in CI mode with strict checks")
