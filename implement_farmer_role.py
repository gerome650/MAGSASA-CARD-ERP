...
def create_farmer_role_and_users(_):
    ...
    try:
        ...
        """,
            (
                "Farmer",
                "Individual farmer with access to personal loan and agricultural information",
                datetime.now(),
            ),
        )
        ...
        for farmer_data in demo_farmers[:5]:  # Limit to 5 demo farmers
            try:
                ...
                if cursor.rowcount > 0:
                    created_users += 1
                    print(
                        f"✅ Created farmer user: {farmer_data['username']} ({farmer_data['full_name']})"
                    )
