...
class LoadTestEngine:
    ...
    async def _worker(self, worker_id: int) -> List[Tuple[float, bool]]:
        ...
        if self.config.request_patterns == "ramp-up":
            ramp_delay = (self.config.ramp_up_time / self.config.concurrency) * worker_id
    ...
    async def run_load_test(self) -> PerformanceMetrics:
        ...
        self.logger.info(f"Starting load test with {self.config.concurrency} concurrent users")
        self.logger.info(f"Target: {self.config.target_url}, Duration: {self.config.duration}s")
        ...
        try:
            ...
            tasks = [
                self._worker(i) for i in range(self.config.concurrency)
            ]
...
async def main():
    ...
    parser.add_argument("--pattern", choices=["burst", "sustained", "ramp-up"],
                       default="sustained", help="Request pattern (default: sustained)")
    parser.add_argument("--config", type=str, default="deploy/performance_config.yml",
                       help="Performance config file path")
    parser.add_argument("--output", type=str, default="deploy/performance_report.md",
