...
AGSCORE_FACTORS = {
    ...
    "Crop Diversification": {
        "weight": 0.15,
        "scoring": {"Single crop": 60, "2 crops": 75, "3 crops": 85, "4+ crops": 95},
    },
    "Land Tenure": {
        "weight": 0.20,
        "scoring": {
    ...
    "Technical Knowledge": {
        "weight": 0.15,
        "scoring": {"Basic": 60, "Intermediate": 75, "Advanced": 90, "Expert": 100},
    },
}
...
def populate_agscore_factors(conn):
    ...
    for factor_name, factor_data in AGSCORE_FACTORS.items():
        cursor.execute(
            ...
            VALUES (?, ?, ?)
        """,
            (factor_name, factor_data["weight"], json.dumps(factor_data["scoring"])),
        )
