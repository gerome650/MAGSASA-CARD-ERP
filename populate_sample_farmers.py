    ...
    with app.app_context():
        ...
        for farmer_data in sample_farmers:
            try:
                ...
                if existing_farmer:
                    print(f"Farmer {farmer_data['full_name']} already exists, skipping...")
                    continue
                ...
                farmer = Farmer(
                    ...
                    date_of_birth=datetime.strptime(farmer_data['date_of_birth'], '%Y-%m-%d').date(),
        ...
        try:
            ...
            for farmer in created_farmers:
                print(f"ID: {farmer.id} | {farmer.full_name} | {farmer.barangay} | AgScore: {farmer.agscore or 'N/A'} | Loan: {farmer.loan_status}")
        ...
        for farmer in farmers_without_agscore:
            try:
                ...
                if agscore_result['success']:
                    ...
                    print(f"  Failed to generate AgScore for {farmer.full_name or farmer.name}")
                    
            except Exception as e:
                print(f"Error updating farmer {farmer.full_name or farmer.name}: {str(e)}")
                continue
        
        try:
            db.session.commit()
            print(f"Successfully updated {len(farmers_without_agscore)} farmers with AgScore!")
        except Exception as e:
...
def print_farmer_statistics():
    ...
    with app.app_context():
        ...
        loan_statuses = db.session.query(Farmer.loan_status, db.func.count(Farmer.id)).group_by(Farmer.loan_status).all()
        print(f"\nLoan Status Distribution:")
        for status, count in loan_statuses:
            print(f"  {status or 'None'}: {count}")
        
        # AgScore grade distribution
        agscore_grades = db.session.query(Farmer.agscore_grade, db.func.count(Farmer.id)).group_by(Farmer.agscore_grade).all()
        print(f"\nAgScore Grade Distribution:")
