...
prequalification_bp = Blueprint(
    "prequalification", __name__, url_prefix="/api/prequalification"
)
...
def assess_farmer():
    ...
    try:
        ...
        return jsonify({"success": True, "assessment": assessment_result})

    except Exception as e:
        logger.error(f"Error in farmer assessment: {e}")
        return (
            jsonify(
                {"error": "Internal server error during assessment", "details": str(e)}
            ),
            500,
        )
    ...
    try:
        ...
        for i, farmer_data in enumerate(farmers):
            try:
                ...
                missing_fields = [
                    field for field in required_fields if not farmer_data.get(field)
                ]
        ...
        summary = {
            ...
            "errors": errors,
            "success_rate": (
                round((total_farmers - errors) / total_farmers * 100, 1)
                if total_farmers > 0
                else 0
            ),
        }
        ...
        return jsonify(
            {
                ...
                "features": {
                    ...
                    "batch_assessment": has_access
                    and user_role in ["manager", "admin", "superadmin"],
                    "templates": has_access,
                    "audit_trail": True,
                },
                "limits": {"max_batch_size": 50, "assessment_timeout": 30},
            }
        )
