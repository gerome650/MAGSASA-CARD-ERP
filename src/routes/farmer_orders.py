...
def create_farmer_order(farmer_id):
    ...
    try:
        ...
        for cart_item in data['cart_items']:
            ...
            if not product:
                return jsonify({'success': False, 'error': f'Product {cart_item["product_id"]} not found'}), 400
...
@farmer_orders_bp.route('/farmers/<int:farmer_id>/orders/<int:order_id>', methods=['GET'])
@require_permission('order_management_read')
...
@farmer_orders_bp.route('/farmers/<int:farmer_id>/orders/<int:order_id>/status', methods=['PUT'])
    ...
    try:
        ...
        return jsonify({
            ...
            'summary': {
                ...
                'item_count': len([item for item in validation_results if item['valid']])
            }
        })
...
def get_farmer_order_summary(farmer_id):
    ...
    try:
        ...
        completed_orders = len([order for order in orders if order.status == 'Delivered'])
