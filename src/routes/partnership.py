...
def get_partner_performance(partner_id):
    try:
        performance_records = PartnerPerformance.query.filter_by(
...
def get_partnership_dashboard():
    try:
        ...
        top_partners = (
            db.session.query(Partner).order_by(Partner.rating.desc()).limit(5).all()
        )
            ...
            export_data.append(
                {
                    ...
                    "Created Date": (
                        partner.created_at.strftime("%Y-%m-%d")
