...
class EnhancedKaAniAssistant:
    ...
    def send_message(self, thread_id, message, role="farmer", user_context=None):
        ...
        try:
            ...
            if not any(
                msg["role"] == "user" and msg["content"] == message
                for msg in api_messages[-3:]
            ):
            ...
            if function_data:
                ...
                function_result = execute_kaani_function(
                    function_data["function"], **function_data["parameters"]
                )
    ...
    def extract_parameters(self, message, function_name):
        ...
        if function_name == "calculate_input_needs":
            ...
            size_match = re.search(
                r"(\d+(?:\.\d+)?)\s*(?:hectare|ha|ektarya)", message_lower
            )
            if size_match:
            ...
            name_match = re.search(
                r"(?:farmer|name|pangalan)\s*:?\s*([a-zA-Z\s]+)", message_lower
            )
            if name_match:
            ...
            location_match = re.search(
                r"(?:location|lugar|address)\s*:?\s*([a-zA-Z\s,]+)", message_lower
            )
            if location_match:
            ...
            amount_match = re.search(
                r"(?:loan|amount|halaga)\s*:?\s*(?:php\s*)?(\d+(?:,\d+)*)",
                message_lower,
            )
            if amount_match:
...
def enhanced_chat():
    ...
    try:
        ...
        # Log the interaction
        logger.info(
            f"Enhanced KaAni - User: {user_name} ({role}), Message: {message[:50]}..."
        )
...
def enhanced_quick_advice():
    ...
    try:
        ...
        response = kaani_enhanced.send_message(
            thread_id=thread["id"], message=message, role=role
        )
