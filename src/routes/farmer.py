...
def get_farmers(_):
    try:
        ...
        return jsonify(
            {
                "farmers": [farmer_item.to_dict() for farmer_item in farmers.items],
                "total": farmers.total,
                "pages": farmers.pages,
                "current_page": page,
                "per_page": per_page,
...
def create_farmer(_):
    try:
        ...
        # Auto-calculate AgScore grade if AgScore is provided and if farmer.agscore:
...
def update_farmer(farmer_id):
    try:
        ...
        # Auto-calculate AgScore grade if AgScore is updated and if "agscore" in data:
...
def update_farmer_agscore(farmer_id):
    try:
        ...
        return jsonify(
            {"message": "AgScore updated successfully", "farmer": farmer.to_dict()}
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        if loan_status not in valid_statuses:
            return (
                jsonify(
                    {
                        "error": f'Invalid loan status. Must be one of: {", ".join(valid_statuses)}'
                    }
                ),
                400,
            )
        ...
        loan_details = {key: v for key, v in data.items() if key != "loan_status"}
        ...
        return jsonify(
            {"message": "Loan status updated successfully", "farmer": farmer.to_dict()}
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def get_farmer_stats(_):
    try:
        total_farmers = Farmer.query.filter(Farmer.is_active).count()

        # Loan status distribution
        loan_stats = (
            db.session.query(
                Farmer.loan_status, db.func.count(Farmer.id).label("count")
            )
        ...
        agscore_stats = (
            db.session.query(
                Farmer.agscore_grade, db.func.count(Farmer.id).label("count")
            )
            .filter(and_(Farmer.is_active, Farmer.agscore_grade.isnot(None)))
        ...
        location_stats = (
            db.session.query(Farmer.barangay, db.func.count(Farmer.id).label("count"))
            .filter(and_(Farmer.is_active, Farmer.barangay.isnot(None)))
            .group_by(Farmer.barangay)
            .order_by(db.func.count(Farmer.id).desc())
            .limit(10)
            .all()
        )
        ...
        return jsonify(
            {
                ...
                "loan_status_distribution": [
                    {"status": stat.loan_status, "count": stat.count}
                    for stat in loan_stats
                ],
                "agscore_distribution": [
                    {"grade": stat.agscore_grade, "count": stat.count}
                    for stat in agscore_stats
                ],
                "top_locations": [
                    {"barangay": stat.barangay, "count": stat.count}
...
@require_permission("farmer_management_read")
def export_farmers(_):
    try:
        format_type = request.args.get("format", "json")

        farmers = Farmer.query.filter(Farmer.is_active).all()
        ...
        if format_type == "csv":
            ...
            return Response(
                ...
                mimetype="text/csv",
                headers={
                    "Content-Disposition": f'attachment; filename=farmers_export_{datetime.now().strftime("%Y%m%d")}.csv'
                },
            )
    ...
    try:
        ...
        if agscore_result["success"]:
            ...
            farmer.update_agscore(
                agscore_result["agscore"], json.dumps(agscore_result["risk_factors"])
            )
            db.session.commit()
...
def get_input_recommendations(farmer_id):
    ...
    try:
        ...
        recommendations_result = ka_ani_service.get_input_recommendations(
            farmer_data, agscore
        )
            ...
            return jsonify(
                {
                    ...
                    "loan_recommendation": recommendations_result[
                        "loan_recommendation"
                    ],
                    "farmer": farmer.to_dict(),
                }
