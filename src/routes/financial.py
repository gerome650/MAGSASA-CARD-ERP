...
def get_financial_dashboard():
    ...
    try:
        ...
        total_cost = (
            db.session.query(func.sum(Order.total_amount * 0.7)).scalar() or 0
        )  # Assuming 70% cost ratio
        ...
        monthly_revenue = (
            ...
            .filter(Order.created_at >= current_month_start)
            .scalar()
            or 0
        )
        ...
        ytd_revenue = (
            db.session.query(func.sum(Order.total_amount))
            .filter(Order.created_at >= year_start)
            .scalar()
            or 0
        )
        ...
        # Average order value
        avg_order_value = (
            total_revenue / Order.query.count() if Order.query.count() > 0 else 0
        )

        # Top revenue generating farmers
        top_farmers = (
            db.session.query(
                Farmer.name, func.sum(Order.total_amount).label("total_spent")
            )
            .join(Order, Farmer.id == Order.farmer_id)
            .group_by(Farmer.id)
        ...
        dashboard_data = {
            ...
            "ytd_metrics": {"revenue": float(ytd_revenue), "orders": ytd_orders},
            "top_farmers": [
                {"name": name, "total_spent": float(total_spent)}
...
def get_profit_loss_statement():
    ...
    try:
        ...
        total_revenue = (
            db.session.query(func.sum(Order.total_amount))
            .filter(and_(Order.created_at >= start_dt, Order.created_at <= end_dt))
            .scalar()
            or 0
        )
        ...
        commission_expenses = (
            ...
            .filter(
                and_(
                    CommissionPayout.created_at >= start_dt,
                    CommissionPayout.created_at <= end_dt,
                )
            )
            .scalar()
            or 0
        ...
        operating_margin = (
            (operating_income / total_revenue * 100) if total_revenue > 0 else 0
        )
        ...
        pl_statement = {
            ...
            "cost_of_goods_sold": {"total_cogs": float(total_cogs)},
            "gross_profit": {
...
def get_margin_analysis():
    ...
    try:
        ...
        for category, data in category_margins.items():
            margin_percentage = (
                (data["total_margin"] / data["total_revenue"] * 100)
...
def get_cash_flow_analysis():
    ...
    try:
        ...
        net_cash_from_operations = (
            cash_from_sales - commission_payments - estimated_operating_expenses
        )
        ...
        while current_date <= end_dt:
            daily_sales = (
                db.session.query(func.sum(Order.total_amount))
                .filter(func.date(Order.created_at) == current_date.date())
                .scalar()
                or 0
            )
...
def get_financial_kpis():
    ...
    try:
        ...
        # Customer metrics
        repeat_customers = (
            db.session.query(func.count(func.distinct(Order.farmer_id)))
            .filter(
                Order.farmer_id.in_(
        ...
        customer_retention_rate = (
            (repeat_customers / total_farmers * 100) if total_farmers > 0 else 0
        )
        ...
        inventory_turnover = (
            total_cogs / total_inventory_value if total_inventory_value > 0 else 0
        )
    ...
    try:
        ...
        if report_type == "profit-loss":
            ...
            writer.writerow(
                [
                    "Operating Expenses",
                    data["operating_expenses"]["total_operating_expenses"],
                ]
            )
