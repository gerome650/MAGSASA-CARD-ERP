...
def get_financial_dashboard():
    ...
    try:
        ...
        monthly_revenue = db.session.query(func.sum(Order.total_amount)).filter(
            Order.created_at >= current_month_start).scalar() or 0
        ...
        ytd_revenue = db.session.query(
            ...
            Order.created_at >= year_start).scalar() or 0
        ...
        top_farmers = db.session.query(
            Farmer.name,
            func.sum(
                Order.total_amount).label('total_spent')).join(
            Order,
            Farmer.id == Order.farmer_id).group_by(
                Farmer.id).order_by(
                    func.sum(
                        Order.total_amount).desc()).limit(5).all()
...
def get_revenue_analysis():
    ...
    try:
        ...
        revenue_by_location = db.session.query(
            Farmer.barangay,
            func.sum(
                Order.total_amount).label('revenue'),
            func.count(
                func.distinct(
                    Farmer.id)).label('farmer_count'),
            func.count(
                    Order.id).label('order_count')).join(
                        Order,
                        Farmer.id == Order.farmer_id).group_by(
                            Farmer.barangay).order_by(
                                func.sum(
                                    Order.total_amount).desc()).limit(10).all()
        ...
        top_products = db.session.query(
            ...
            func.sum(
                Order.total_amount).label('revenue'),
            func.count(
                Order.id).label('order_count'),
            func.avg(
                Order.total_amount).label('avg_order_value')).join(
                    Order,
                    Product.id == Order.product_id).group_by(
                        Product.id).order_by(
                            func.sum(
                                Order.total_amount).desc()).limit(10).all()
...
def get_cash_flow_analysis():
    ...
    try:
        ...
        commission_payments = db.session.query(
            func.sum(
                CommissionPayout.amount)).filter(
            and_(
                CommissionPayout.created_at >= start_dt,
                CommissionPayout.created_at <= end_dt,
                CommissionPayout.status == 'Paid')).scalar() or 0
...
def get_financial_kpis():
    ...
    try:
        ...
        monthly_growth_rate = ((current_month_revenue - last_month_revenue) /
