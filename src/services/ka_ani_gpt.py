...
class KaAniGPTService:
    ...
    def prequalify_farmer(self, farmer_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            response = self._call_ka_ani_api(
                "prequalify", {"prompt": prompt, "farmer_data": farmer_data}
            )
    ...
    def calculate_agscore(self, farmer_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Make API call to Ka-Ani GPT
            response = self._call_ka_ani_api(
                "agscore", {"prompt": prompt, "farmer_data": farmer_data}
            )
    ...
    def get_input_recommendations(
        self, farmer_data: dict[str, Any], agscore: int
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            response = self._call_ka_ani_api(
                ...
                {"prompt": prompt, "farmer_data": farmer_data, "agscore": agscore},
            )
    ...
    def _build_prequalification_prompt(self, farmer_data: dict[str, Any]) -> str:
        ...
        location = farmer_data.get("farm_location_text") or farmer_data.get(
            "location", "Unknown"
        )
    ...
    def _build_input_recommendations_prompt(
        self, farmer_data: dict[str, Any], agscore: int
    ) -> str:
    ...
    def _generate_mock_prequalification(
        self, farmer_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if land_size >= 2 and experience >= 3:
            ...
            recommendations = (
                "Farmer may need additional support or training before loan approval."
            )
    ...
    def _calculate_mock_agscore(self, farmer_data: dict[str, Any]) -> dict[str, Any]:
        ...
        crop = (
            farmer_data.get("crop_types") or farmer_data.get("crop_type", "")
        ).lower()
        ...
        risk_factors = {
            "weather_risk": (
                "Low" if agscore > 80 else "Medium" if agscore > 60 else "High"
            ),
            ...
            "farmer_experience": (
                "High" if experience > 5 else "Medium" if experience > 2 else "Low"
            ),
        }
    ...
    def _generate_mock_input_recommendations(
        self, farmer_data: dict[str, Any], agscore: int
    ) -> dict[str, Any]:
        """Generate mock input recommendations"""
        land_size = farmer_data.get("land_size_ha", 1)
        crop = (
            farmer_data.get("crop_types") or farmer_data.get("crop_type", "rice")
        ).lower()
