...
def verify_data():
    ...
    cursor.execute("SELECT first_name, last_name, email, crop_type, loan_amount FROM farmers WHERE notes LIKE '%FICTITIOUS DATA%' LIMIT 5")
    ...
    cursor.execute("SELECT crop_type, COUNT(*) FROM farmers WHERE notes LIKE '%FICTITIOUS DATA%' GROUP BY crop_type ORDER BY COUNT(*) DESC")
