    ...
    with app.app_context():
        ...
        for partner in partners[:5]:  # Add performance for first 5 partners
            ...
            performance_data.append(
                PartnerPerformance(
                    ...
                    metric_value=partner.successful_deliveries
                    / max(partner.total_orders, 1)
                    * 100,
        ...
        for partner in partners[:3]:  # Add contracts for first 3 partners
            contract_data.append(
                PartnerContract(
                    ...
                    contract_value=(
                        500000.0 if partner.partner_type == "Supplier" else 100000.0
                    ),
        ...
        for partner in partners[:4]:  # Add commissions for first 4 partners
            commission_data.append(
                CommissionPayout(
                    partner_id=partner.id,
                    amount=partner.total_commission_earned
                    * 0.3,  # 30% of total as recent payout
                    commission_rate=partner.commission_rate,
                    base_amount=partner.total_commission_earned
                    * 0.3
        ...
        print(
            f"Supplier Partners: {Partner.query.filter_by(partner_type='Supplier').count()}"
        )
        print(
            f"Logistics Partners: {Partner.query.filter_by(partner_type='Logistics').count()}"
        )
        print(f"Active Partners: {Partner.query.filter_by(status='Active').count()}")

        total_commissions = (
            db.session.query(db.func.sum(CommissionPayout.amount)).scalar() or 0
        )
        pending_commissions = (
            db.session.query(db.func.sum(CommissionPayout.amount))
