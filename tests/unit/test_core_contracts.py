...
class TestAgentInput:
    ...
    def test_agent_input_creation(self):
        ...
        data = AgentInput(
            request_id="test-123", agent_type="test-agent", payload={"key": "value"}
        )
...
class TestAgentProtocol:
    ...
    async def test_agent_protocol_abstract_methods(self):
        ...
        class TestAgent(AgentProtocol):
            ...
            async def health_check(self):
                raise NotImplementedError("health_check method not implemented")

            async def validate_input(self, input_data):
                raise NotImplementedError("validate_input method not implemented")

        agent = TestAgent()

        with pytest.raises(NotImplementedError):
            await agent.run(
                AgentInput(request_id="test", agent_type="test", payload={})
            )
