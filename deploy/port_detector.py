...
class PortDetector:
    ...
    def detect_flask_port(self, host: str = "localhost") -> tuple[int, dict] | None:
        """
        ...
        if process_ports:
            ...
            for port in process_ports:
                ...
                if is_healthy:
                    print(
                        f"   ✅ Found healthy service on port {port}", file=sys.stderr
                    )
                    return port, info
        ...
        if env_port:
            try:
                ...
                if is_healthy:
                    print(
                        f"   ✅ Found healthy service on environment port {port}",
                        file=sys.stderr,
                    )
                    return port, info
            except ValueError:
    ...
    def get_target_url(
        self, host: str = "localhost", fallback_port: int = 8000
    ) -> tuple[str, bool]:
        """
...
def main(_):
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress detection messages"
    )

    args = parser.parse_args()

    # Redirect stderr to devnull if quiet and if args.quiet:
