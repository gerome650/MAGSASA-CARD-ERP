...
class ResilienceMetrics:
    ...
    # Recovery metrics
    mttr: float | None = None  # Mean Time To Recovery in seconds
    recovery_time: float | None = None  # Time to full recovery
    ...
    chaos_latency_ms: float | None = None
    post_chaos_latency_ms: float | None = None
    latency_degradation_ms: float | None = None

    # Additional metrics
    health_check_failures: int = 0
    consecutive_successes: int = 0
    first_success_after_chaos: float | None = None
...
class ResilienceValidator:
    ...
    def __init__(
        self, target_url: str = None, config_path: str = "deploy/chaos_scenarios.yml"
    ):
    ...
    def _load_slo_config(self) -> tuple[SLOTargets, SLOSeverity | None]:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                slo_targets = SLOTargets(
                    ...
                    min_availability_percent=targets.get(
                        "min_availability_percent", 92.0
                    ),
                    max_latency_degradation_ms=targets.get(
                        "max_latency_degradation_ms", 800.0
                    ),
                    max_recovery_time_seconds=recovery.get(
                        "max_recovery_time_seconds", 15.0
                    ),
                    required_health_checks=recovery.get("required_health_checks", 3),
                    health_check_interval_seconds=recovery.get(
                ...
                if severity_config:
                    ...
                    blocking_targets = SLOTargets(
                        ...
                        max_error_rate_percent=blocking_config.get(
                            "max_error_rate_percent", 15.0
                        ),
                        min_availability_percent=blocking_config.get(
                            "min_availability_percent", 85.0
                        ),
                        max_latency_degradation_ms=blocking_config.get(
                            "max_latency_degradation_ms", 1500.0
                        ),
                        max_recovery_time_seconds=blocking_config.get(
                    ...
                    warning_targets = SLOTargets(
                        ...
                        max_error_rate_percent=warning_config.get(
                            "max_error_rate_percent", 8.0
                        ),
                        min_availability_percent=warning_config.get(
                            "min_availability_percent", 92.0
                        ),
                        max_latency_degradation_ms=warning_config.get(
                            "max_latency_degradation_ms", 800.0
                        ),
                        max_recovery_time_seconds=warning_config.get(
                            "max_recovery_time_seconds", 15.0
                        ),
                    )

                    slo_severity = SLOSeverity(
                        blocking=blocking_targets, warning=warning_targets
                    )
            ...
            self.logger.warning(
                f"Could not load SLO configuration from {self.config_path}: {e}"
            )

        return SLOTargets(), None

    async def wait_for_service_ready(
        self, timeout: int = 60, interval: float = 2.0
    ) -> bool:
        """
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                            ...
                            async with session.get(
                                ...
                                if resp.status == 200:
                                    ...
                                    if (
                                        ...
                                        self.logger.info(
                                            f"✅ Service is ready (attempt {attempts})"
                                        )
                                        return True
                        except Exception:
                ...
                self.logger.debug(
                    f"Service readiness check failed (attempt {attempts}): {e}"
                )

            self.logger.debug(
                f"   Attempt {attempts} - service not ready, waiting {interval}s..."
            )
    ...
    async def monitor_during_chaos(
        self, duration: int = 60, interval: float = 2.0
    ) -> dict:
        ...
        self.metrics.error_rate_percent = (
            (failures / total_checks * 100) if total_checks > 0 else 0
        )
        self.metrics.uptime_seconds = uptime
        self.metrics.downtime_seconds = total_downtime
        self.metrics.availability_percent = (
            (uptime / actual_duration * 100) if actual_duration > 0 else 0
        )
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if self.metrics.first_success_after_chaos is None:
                                self.metrics.first_success_after_chaos = (
                            ...
                            if consecutive_successes >= required_successes:
                                ...
                                self.logger.info(
                                    f"   ✓ System recovered in {recovery_time:.1f}s"
                                )
    ...
    async def measure_post_chaos_latency(self, samples: int = 10) -> float:
        ...
        if latencies:
            ...
            if self.metrics.baseline_latency_ms:
                self.metrics.latency_degradation_ms = (
                    post_latency - self.metrics.baseline_latency_ms
                )
    ...
    def calculate_mttr(
        self, chaos_results_file: str = "deploy/chaos_results.json"
    ) -> float | None:
        ...
        try:
            ...
            if not os.path.exists(chaos_results_file):
                self.logger.warning(
                    f"Chaos results file not found: {chaos_results_file}"
                )
            ...
            with open(chaos_results_file) as f:
                results = json.load(f)
    ...
    def _find_latest_chaos_results(self) -> str | None:
    ...
    async def estimate_availability_from_readiness(
        self, duration: int = 30, interval: float = 2.0
    ) -> float:
        """
        ...
        """
        self.logger.info(
            f"📊 Estimating availability from readiness checks ({duration}s)"
        )
        ...
        if total_checks > 0:
            ...
            self.logger.info(
                f"   Estimated availability: {availability:.1f}% ({successful_checks}/{total_checks} checks)"
            )
            return availability
        else:
    ...
    def validate_slos(self) -> ValidationResult:
        ...
        blocking_targets = (
            self.slo_severity.blocking if self.slo_severity else self.slo_targets
        )
        warning_targets = (
            self.slo_severity.warning if self.slo_severity else self.slo_targets
        )
        ...
        if self.metrics.mttr is not None:
            ...
            self.logger.info(
                f"   MTTR: {self.metrics.mttr:.1f}s / {warning_targets.mttr_seconds:.1f}s warning / {blocking_targets.mttr_seconds:.1f}s blocking"
            )
        ...
        if (
            ...
            if (
                ...
                self.logger.warning(
                    f"   Using conservative availability estimate: {availability_to_check:.1f}%"
                )
        ...
        if self.metrics.latency_degradation_ms is not None:
            if (
                self.metrics.latency_degradation_ms
                > blocking_targets.max_latency_degradation_ms
            ):
                violations.append(
                ...
                self.metrics.latency_degradation_ms
                > warning_targets.max_latency_degradation_ms
            ):
                warnings.append(
    ...
    def generate_report(
        self, result: ValidationResult, output_file: str = "deploy/chaos_report.md"
    ) -> str:
...
"""
        ...
        if result.metrics.mttr is not None:
            mttr_status = (
                "✅" if result.metrics.mttr <= result.slo_targets.mttr_seconds else "❌"
            )
        ...
        if result.metrics.recovery_time is not None:
            recovery_status = (
                "✅"
                if result.metrics.recovery_time
                <= result.slo_targets.max_recovery_time_seconds
                else "❌"
            )
        ...
        error_status = (
            "✅"
            if result.metrics.error_rate_percent
            <= result.slo_targets.max_error_rate_percent
            else "❌"
        )
        ...
        availability_note = ""
        if (
            result.metrics.availability_percent == 0.0
            and result.metrics.total_requests == 0
        ):
            if (
                result.metrics.consecutive_successes
                >= result.slo_targets.required_health_checks
            ):
        ...
        avail_status = (
            "✅"
            if availability_display >= result.slo_targets.min_availability_percent
            else "❌"
        )
        ...
        if result.metrics.latency_degradation_ms is not None:
            latency_status = (
                "✅"
                if result.metrics.latency_degradation_ms
                <= result.slo_targets.max_latency_degradation_ms
                else "❌"
            )
...
"""
        ...
        if result.passed:
            ...
            if (
                result.metrics.mttr
                and result.metrics.mttr > result.slo_targets.mttr_seconds
            ):
                report += "- **Improve Recovery Time:** Implement faster failure detection and automated recovery\n"

            if (
                result.metrics.error_rate_percent
                > result.slo_targets.max_error_rate_percent
            ):
                report += "- **Reduce Error Rate:** Add circuit breakers and better error handling\n"

            if (
                result.metrics.availability_percent
                < result.slo_targets.min_availability_percent
            ):
                report += "- **Increase Availability:** Implement redundancy and failover mechanisms\n"

            if (
                result.metrics.latency_degradation_ms
                and result.metrics.latency_degradation_ms
                > result.slo_targets.max_latency_degradation_ms
            ):
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Resilience Validator - Stage 6.5")
    parser.add_argument(
        "--target",
        type=str,
        default=None,
    ...
    # Create validator
    validator = ResilienceValidator(target_url=args.target, config_path=args.config)

    print("🧪 Starting resilience validation")
