...
def debug_database():
    ...
    try:
        ...
        if total_farmers > 0:
            # Show sample data
            cursor.execute("SELECT id, full_name, address, crop_types, loan_status, agscore FROM farmers LIMIT 5;")
            sample_farmers = cursor.fetchall()
            print("\n👥 Sample farmers:")
            for farmer in sample_farmers:
                print(f"  ID: {farmer[0]}, Name: {farmer[1]}, Location: {farmer[2][:50]}...")
                print(f"      Crops: {farmer[3]}, Loan: {farmer[4]}, AgScore: {farmer[5]}")
