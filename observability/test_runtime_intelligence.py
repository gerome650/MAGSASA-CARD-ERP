...
from alerts.notifier import SmartAlertRouter
from alerts.webhook_server import process_alert
from dashboards.annotations import AnnotationManager, GrafanaAnnotationService
...
class RuntimeIntelligenceTester:
    ...
    def test_alertmanager_connectivity(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.alertmanager_url}/api/v1/status", timeout=10
            )
            response.raise_for_status()

            status_data = response.json()
            logger.info(
                f"✓ Alertmanager status: {status_data.get('data', {}).get('cluster', {}).get('status', 'unknown')}"
            )
    ...
    def test_anomaly_detectors(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ EWMA detector processed {len(normal_values)} values, detected {anomalies_detected} anomalies"
            )
            ...
            if anomaly_result and anomaly_result.is_anomaly:
                logger.info(
                    f"✓ EWMA detector correctly identified anomaly: {anomaly_result.anomaly_score:.2f}"
                )
            else:
            ...
            if zscore_result and zscore_result.is_anomaly:
                logger.info(
                    f"✓ Z-Score detector correctly identified anomaly: {zscore_result.anomaly_score:.2f}"
                )
            else:
                logger.warning("⚠ Z-Score detector did not detect expected anomaly")

            # Test Rolling Percentile detector
            percentile_detector = RollingPercentileDetector(
                window_size=50, percentile=95.0, threshold_multiplier=2.0
            )
            ...
            if percentile_result and percentile_result.is_anomaly:
                logger.info(
                    f"✓ Rolling Percentile detector correctly identified latency anomaly: {percentile_result.anomaly_score:.2f}"
                )
            else:
                logger.warning(
                    "⚠ Rolling Percentile detector did not detect expected latency anomaly"
                )
    ...
    def test_runtime_intelligence_engine(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"⚠ Engine anomaly check failed (expected if Prometheus not available): {e}"
                )
    ...
    def test_alert_routing(self) -> bool:
        ...
        try:
            ...
            test_config = {
                ...
                "pagerduty": {"enabled": False, "integration_key": "test-key"},
                "email": {"enabled": False, "smtp_host": "test.example.com"},
            }
            ...
            logger.info(
                f"✓ Router initialized with {len(stats['configured_channels'])} channels"
            )
            ...
            logger.info(
                f"✓ Alert context extracted: severity={context.severity.value}, service={context.service}"
            )
    ...
    def test_dashboard_annotations(self) -> bool:
        ...
        try:
            ...
            annotation_service = GrafanaAnnotationService(
                grafana_url=self.grafana_url, api_key="test-api-key"
            )
            ...
            try:
                ...
                logger.info(
                    f"✓ Dashboard service initialized, found {len(dashboards)} dashboards"
                )
            except Exception as e:
                logger.warning(
                    f"⚠ Dashboard listing failed (expected with dummy API key): {e}"
                )
            ...
            logger.info(
                f"✓ Annotation manager initialized with {mapping_stats['total_mappings']} mappings"
            )
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"⚠ Webhook server health check returned {response.status_code}"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠ Webhook server not accessible (expected if not running): {e}"
                )
    ...
    def test_promql_rules_validation(self) -> bool:
        ...
        try:
            ...
            rules_file = os.path.join(
                os.path.dirname(__file__), "alerts", "promql_rules.yml"
            )
            ...
            if missing_sections:
                logger.error(
                    f"✗ Alert rules missing required sections: {missing_sections}"
                )
                self.test_results["promql_rules_validation"] = False
                return False
            ...
            logger.info(
                f"✓ Alert rules file validated: {alert_count} alerts, {expr_count} expressions"
            )
            ...
            for query in test_queries:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"⚠ PromQL query returned {response.status_code}: {query[:50]}..."
                        )
                except Exception as e:
                    logger.warning(
                        f"⚠ PromQL query test failed (expected if Prometheus not available): {e}"
                    )
    ...
    def run_comprehensive_test(self) -> dict[str, Any]:
        ...
        if passed_tests == total_tests:
            logger.info(
                "🎉 ALL TESTS PASSED! Runtime Intelligence is ready for production."
            )
        else:
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if not self.test_results.get("prometheus_connectivity"):
            recommendations.append(
                "Start Prometheus server and ensure metrics are being collected"
            )
        ...
        if not self.test_results.get("grafana_connectivity"):
            recommendations.append(
                "Start Grafana server and configure API key for annotations"
            )

        if not self.test_results.get("webhook_server"):
            recommendations.append(
                "Start the webhook server: python -m observability.alerts.webhook_server"
            )
        ...
        if len(recommendations) == 0:
            recommendations.append(
                "All components are working correctly! Consider running load tests."
            )
...
def main():
    ...
    parser.add_argument(
        "--grafana-url", default="http://localhost:3000", help="Grafana server URL"
    )
    parser.add_argument(
        "--webhook-url", default="http://localhost:5001", help="Webhook server URL"
    )
