...
class RemediationAdvisor:
    ...
    def generate_remediation_plan(
        self, insight: IncidentInsight, available_automation: list[str] = None
        ...
        if not insight.likely_root_causes:
            ...
            for root_cause in insight.likely_root_causes:
                actions = self._generate_remediation_for_cause(
                    root_cause, insight, available_automation
                )
        ...
        remediation_actions.sort(
            key=lambda x: (priority_order[x.priority], x.expected_duration_minutes)
        )

        logger.info(
            f"Generated {len(remediation_actions)} remediation actions for incident {insight.incident_id}"
        )
