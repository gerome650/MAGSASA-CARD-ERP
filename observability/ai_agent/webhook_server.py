...
agent: Optional[AIIncidentAgent] = None
...
async def startup_event():
    ...
    if not config_path.exists():
        ...
        config = AgentConfig(
            prometheus_url=config_data.get("data_sources", {}).get("prometheus", {}).get("base_url", "http://localhost:9090"),
            jaeger_url=config_data.get("data_sources", {}).get("jaeger", {}).get("base_url", "http://localhost:16686"),
            loki_url=config_data.get("data_sources", {}).get("loki", {}).get("base_url", "http://localhost:3100"),
            slack_bot_token=config_data.get("notifications", {}).get("slack", {}).get("bot_token"),
            slack_channels=config_data.get("notifications", {}).get("slack", {}).get("channels", {}),
            pagerduty_token=config_data.get("notifications", {}).get("pagerduty", {}).get("api_token"),
            pagerduty_integration_keys=config_data.get("notifications", {}).get("pagerduty", {}).get("integration_keys", {}),
            reports_dir=config_data.get("postmortem", {}).get("reports_directory", "/observability/reports"),
            analysis_window_minutes=config_data.get("analysis", {}).get("window_minutes", 30),
            confidence_threshold=config_data.get("analysis", {}).get("confidence_threshold", 0.3)
        )
...
async def get_postmortem(incident_id: str):
    ...
    try:
        ...
        postmortem_path = Path(agent.config.reports_dir) / f"{incident_date}-incident-{incident_id}.md"
...
async def analyze_incident_background(
    ...
    try:
        ...
        logger.info(f"Incident {incident_id} - Impact: {insight['business_impact']}, "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI Incident Insight Agent Webhook Server")
