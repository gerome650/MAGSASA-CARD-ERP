...
from .data_collector import IncidentContextCollector, IncidentContext
...
class AIIncidentAgent:
    ...
    async def analyze_incident(
        ...
        try:
            ...
            context = await self.data_collector.collect_incident_context(
                incident_id, 
                alert_payload, 
                self.config.analysis_window_minutes
            )
            ...
            root_causes = [cause for cause in root_causes if cause.confidence >= self.config.confidence_threshold]
            ...
            remediation_actions = self.remediation_advisor.generate_remediation_plan(insight)
            
            # Step 5: Generate reports for all channels
            logger.info("Step 5: Generating incident reports")
            reports = self.incident_reporter.generate_reports(insight, remediation_actions)
            
            # Step 6: Send notifications
            logger.info("Step 6: Sending notifications")
            notification_results = await self._send_notifications(insight, remediation_actions, reports)
    ...
    def _get_reporter_config(self) -> Dict[str, Any]:
        ...
        if self.config.slack_bot_token and self.config.slack_channels:
            channels.append({
                "type": "slack",
                "destination": self.config.slack_channels.get("incidents", "#incident-response"),
