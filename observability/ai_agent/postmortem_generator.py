...
class PostmortemGenerator:
    ...
    def _generate_sections(
        ...
        # Resolution section
        sections.append(
            self._generate_resolution_section(
                insight, remediation_actions, resolution_notes
            )
        )

        # Lessons learned section
        sections.append(
            self._generate_lessons_learned_section(insight, remediation_actions)
        )
...
"""
        ...
        return PostmortemSection(title="Summary", content=content.strip(), order=1)
    ...
    def _generate_root_causes_section(
        ...
        if not insight.likely_root_causes:
            ...
            return PostmortemSection(
                title="Root Causes", content=content.strip(), order=3
            )
...
"""
        ...
        return PostmortemSection(
            title="Impact Analysis", content=content.strip(), order=4
        )
        ...
        if insight.likely_root_causes:
            ...
            if primary_cause.cause_type.value == "deployment_regression":
                detection_methods.append(
                    "Deployment monitoring and automated rollback triggers"
                )
                ...
                detection_methods.append(
                    "Infrastructure monitoring and resource utilization alerts"
                )
            elif primary_cause.cause_type.value == "dependency_failure":
                detection_methods.append(
                    "External service monitoring and circuit breaker alerts"
                )
            elif primary_cause.cause_type.value == "resource_exhaustion":
                detection_methods.append(
                    "Resource utilization monitoring and memory leak detection"
                )
...
"""
        ...
        return PostmortemSection(title="Detection", content=content.strip(), order=5)
    ...
    def _generate_resolution_section(
        ...
        if not remediation_actions:
            ...
            return PostmortemSection(
                title="Resolution", content=content.strip(), order=6
            )

        # Group actions by priority
        immediate_actions = [
            a for a in remediation_actions if a.priority.value == "immediate"
        ]
        high_priority_actions = [
            a for a in remediation_actions if a.priority.value == "high"
        ]
        other_actions = [
            a
            for a in remediation_actions
            if a.priority.value not in ["immediate", "high"]
        ]
...
"""
        ...
        return PostmortemSection(
            title="Lessons Learned", content=content.strip(), order=7
        )
    ...
    def _generate_action_items_section(
        ...
        if not remediation_actions:
            ...
            return PostmortemSection(
                title="Action Items", content=content.strip(), order=8
            )
...
"""
        ...
        for item in action_items:
            automation_note = (
                " (Automation recommended)" if item["automation_possible"] else ""
            )
            content += f"""
...
"""
    ...
    def _generate_engineer_notes_section(
        self, engineer_notes: str
    ) -> PostmortemSection:
...
"""
        ...
        return PostmortemSection(
            title="Engineer Notes", content=content.strip(), order=9
        )
...
""".format(
        ...
        return PostmortemSection(
            title="Metrics and Monitoring", content=content.strip(), order=10
        )
