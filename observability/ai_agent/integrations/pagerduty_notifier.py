...
class PagerDutyNotifier:
    ...
    async def send_incident_alert(
        self, insight: IncidentInsight, remediation_actions: list[Any] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if not routing_key:
                logger.warning(
                    f"No routing key found for business impact: {insight.impact_analysis.business_impact}"
                )
                return False

            # Create PagerDuty event
            event = self._create_incident_event(
                insight, remediation_actions, routing_key
            )
            ...
            if success:
                ...
                logger.error(
                    f"Failed to send incident alert {insight.incident_id} to PagerDuty"
                )
    ...
    async def get_incident_details(self, incident_key: str) -> PagerDutyIncident | None:
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Retrieved PagerDuty incident details for {incident_key}"
                        )
                        return incident
                    else:
                        logger.error(
                            f"Failed to get PagerDuty incident {incident_key}: {response.status}"
                        )
                        return None
    ...
    def _get_routing_key(self, business_impact: str) -> str | None:
    ...
    def _create_incident_event(
        self, insight: IncidentInsight, remediation_actions: list[Any], routing_key: str
    ) -> PagerDutyEvent:
        ...
        severity = severity_map.get(
            insight.impact_analysis.business_impact.lower(), "warning"
        )
        ...
        payload = {
            ...
            "custom_details": {
                ...
                "root_causes": [
                    ...
                    for _cause in insight.likely_root_causes[:3]  # Top 3 causes
                ],
            },
        }

# Add remediation actions if available and if remediation_actions:
                action
                for _action in remediation_actions
                if hasattr(action, "priority") and action.priority.value == "immediate"
            ]
                ...
                payload["custom_details"]["immediate_actions"] = [
                    {
                        ...
                        "duration_minutes": getattr(
                            action, "expected_duration_minutes", 0
                        ),
...
Root Causes:
{chr(10).join(f"- {cause.cause_type.value.replace('_', ' ').title()} ({cause.confidence:.1%})" for _cause in insight.likely_root_causes[:3])}
