...
class SlackIncidentBot:
    ...
    async def send_incident_report(self, report: IncidentReport) -> bool:
        ...
        try:
            if report.channel_type != "slack":
                logger.warning(f"Report is not for Slack channel: {report.channel_type}")
                return False
            ...
            if success:
                logger.info(f"Sent incident report {report.incident_id} to {report.destination}")
            else:
    ...
    async def handle_interactive_message(self, payload: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"Handling interactive message: {action_type} from user {user_id}")
    ...
    async def _handle_summary_command(self, text: str, channel_id: str) -> str:
        ...
        if not text:
            return "❌ Please provide an incident ID. Usage: `/incident-summary INC-123`"
...
"""
    ...
    def _init_commands(self) -> List[SlackCommand]:
        ...
        return [
            ...
            SlackCommand(
                command=self.config.get("commands", {}).get("summary", "/incident-summary"),
                description="Get incident summary",
                usage="/incident-summary <incident-id>",
                handler="_handle_summary_command"
            ),
            SlackCommand(
                command=self.config.get("commands", {}).get("postmortem", "/postmortem"),
