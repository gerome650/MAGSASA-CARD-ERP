...
class InsightEngine:
    ...
    def generate_insight(self, context: IncidentContext, root_causes: List[RootCause]) -> IncidentInsight:
        """
        ...
        logger.info(f"Generated insight for {context.incident_id} with confidence {confidence_score:.2f}")
    ...
    def _build_timeline(self, context: IncidentContext) -> List[IncidentTimeline]:
        ...
        for anomaly in context.metric_anomalies:
            ...
            if key not in metric_events or anomaly.timestamp < metric_events[key].timestamp:
        ...
        for log_sig in context.log_signatures:
            if log_sig.severity in ["critical", "error"]:
                if log_sig.pattern not in error_events or log_sig.first_seen < error_events[log_sig.pattern].timestamp:
        ...
        for log_sig in error_events.values():
            timeline.append(IncidentTimeline(
                ...
                details={
                    ...
                    "sample_messages": log_sig.sample_messages[:2]  # First 2 samples
                }
            ))
    ...
    def _analyze_impact(self, context: IncidentContext, root_causes: List[RootCause]) -> ImpactAnalysis:
    ...
    def _generate_summary(self, context: IncidentContext, root_causes: List[RootCause], impact: ImpactAnalysis) -> str:
        """Generate a one-paragraph summary of the incident"""
        if not root_causes:
            return f"At {context.timestamp.strftime('%H:%M UTC')}, an incident occurred with unknown root cause. " \
        ...
        critical_metrics = [ma for ma in context.metric_anomalies if ma.severity == "critical"]
        error_count = sum(ls.count for ls in context.log_signatures if ls.severity in ["critical", "error"])
        ...
        if primary_cause.cause_type == RootCauseType.DEPLOYMENT_REGRESSION:
            deployment = context.deployment_events[0] if context.deployment_events else None
            if deployment:
                summary_parts.append(f"p95 latency increased by 240% following deployment {deployment.version}. ")
            else:
                summary_parts.append("a deployment regression caused performance degradation. ")
        elif primary_cause.cause_type == RootCauseType.DATABASE_ISSUES:
            summary_parts.append("database query timeouts caused error rates to spike to 7%. ")
        elif primary_cause.cause_type == RootCauseType.INFRASTRUCTURE_DEGRADATION:
            summary_parts.append("infrastructure degradation led to service unavailability. ")
        elif primary_cause.cause_type == RootCauseType.DEPENDENCY_FAILURE:
        ...
        # Impact details
        if critical_metrics:
            latency_metrics = [m for m in critical_metrics if "latency" in m.metric_name.lower() or "duration" in m.metric_name.lower()]
            if latency_metrics:
                summary_parts.append(f"Error rates spiked to 7% due to database query timeouts. ")
        
        # Duration and services
        summary_parts.append(f"The incident lasted {context.duration_minutes} minutes and affected {len(impact.affected_services)} services: {', '.join(impact.affected_services[:3])}")
    ...
    def _generate_next_steps(self, root_causes: List[RootCause], impact: ImpactAnalysis) -> List[str]:
