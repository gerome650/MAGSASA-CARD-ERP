...
class IncidentAgentTester:
    ...
    def _create_dependency_failure_scenario(self) -> TestScenario:
        ...
        return TestScenario(
            ...
            alert_payload={
                ...
                "commonLabels": {"service": "magsasa-card-erp"},
                "commonAnnotations": {"summary": "External payment service unavailable"},
                "externalURL": "http://localhost:9093",
                "version": "4",
                "groupKey": "{}:{}"
            },
    ...
    async def run_test_suite(self) -> Dict[str, Any]:
        ...
        results["success_rate"] = results["passed_scenarios"] / results["total_scenarios"] if results["total_scenarios"] > 0 else 0
        
        logger.info(f"Test suite completed. Success rate: {results['success_rate']:.1%}")
        return results
    ...
    def _validate_scenario_results(self, scenario: TestScenario, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        expected_causes_found = any(
            expected in found_root_causes 
            for expected in scenario.expected_root_causes
        )
        ...
        action_types = [action.get("action_type", "") for action in results["remediation_actions"]]
        expected_actions_found = any(
            expected in action_types 
    ...
    async def _test_insight_engine(self) -> Dict[str, Any]:
        ...
        try:
            ...
            root_causes = self.agent.incident_analyzer.analyze_incident(test_context)
            insight = self.agent.insight_engine.generate_insight(test_context, root_causes)
