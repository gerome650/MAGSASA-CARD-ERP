...
class IncidentReporter:
    ...
    def generate_reports(
        ...
        for channel in self.channels:
            # Check if this channel should receive this incident
            if self._should_notify_channel(channel, insight, remediation_actions):
                report = self._generate_channel_report(channel, insight, remediation_actions)
                reports.append(report)
        
        logger.info(f"Generated {len(reports)} reports for incident {insight.incident_id}")
    ...
    def _generate_channel_report(
        ...
        if channel.channel_type == "slack":
            ...
            return self._generate_dashboard_report(channel, insight, remediation_actions)
        else:
    ...
    def _generate_slack_report(
        ...
        immediate_actions = [action for action in remediation_actions if action.priority.value == "immediate"]
        if immediate_actions:
            actions_text = ""
            for action in immediate_actions[:3]:  # Top 3 immediate actions
                actions_text += f"• *{action.title}* ({action.expected_duration_minutes}min)\n"
        ...
        if insight.timeline:
            ...
            for event in insight.timeline[:5]:  # First 5 events
                timeline_text += f"• {event.timestamp.strftime('%H:%M')} - {event.description}\n"
