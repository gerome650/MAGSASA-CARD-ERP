...
def create_agent_config(config: dict[str, Any]) -> AgentConfig:
    ...
    return AgentConfig(
        # Data sources
        prometheus_url=data_sources.get("prometheus", {}).get(
            "base_url", "http://localhost:9090"
        ),
        jaeger_url=data_sources.get("jaeger", {}).get(
        ...
        # Notifications
        slack_bot_token=(
            notifications.get("slack", {}).get("bot_token")
            if notifications.get("slack", {}).get("enabled")
            else None
        ),
        slack_channels=notifications.get("slack", {}).get("channels", {}),
        pagerduty_token=(
            notifications.get("pagerduty", {}).get("api_token")
            if notifications.get("pagerduty", {}).get("enabled")
            else None
        ),
        pagerduty_integration_keys=notifications.get("pagerduty", {}).get(
            "integration_keys", {}
        ),
        # Analysis
        reports_dir=config.get("postmortem", {}).get(
            "reports_directory", "/observability/reports"
        ),
        analysis_window_minutes=config.get("analysis", {}).get("window_minutes", 30),
        confidence_threshold=config.get("analysis", {}).get(
...
Examples:
  ...
  python -m observability.ai_agent.cli postmortem --incident-id INC-001 --incident-file incident.json
    ...
    test_parser.add_argument(
        "--scenarios", action="store_true", help="Run test scenarios"
    )
    test_parser.add_argument(
    ...
    postmortem_parser.add_argument(
        "--incident-file", required=True, help="Incident data JSON file"
    )
