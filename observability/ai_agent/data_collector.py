...
class IncidentContextCollector:
    ...
    async def collect_incident_context(
        ...
        alert_time = datetime.fromisoformat(
            alert_payload.get("alerts", [{}])[0].get("startsAt", datetime.now().isoformat())
        )
        ...
        logger.info(f"Collected context for {incident_id}: {len(metric_anomalies)} metrics, "
                ...
                if data.get("status") == "success":
                    for result in data.get("data", {}).get("result", []):
                        ...
                        anomaly = MetricAnomaly(
                            ...
                            timestamp=datetime.fromtimestamp(result.get("value", [0, 0])[0]),
                            labels=metric,
                    ...
                    for span in trace.get("spans", []):
                        ...
                        if duration_ms > 2000:  # > 2 seconds
                            outlier = TraceOutlier(
                                ...
                                timestamp=datetime.fromtimestamp(span.get("startTime", 0) / 1000000),
