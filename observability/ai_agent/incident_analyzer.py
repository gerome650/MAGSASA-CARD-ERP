...
class IncidentAnalyzer:
    ...
    def analyze_incident(self, context: IncidentContext) -> list[RootCause]:
        ...
        logger.info(
            f"Found {len(root_causes)} potential root causes for incident {context.incident_id}"
        )

        return root_causes

    def _analyze_deployment_regression(
        ...
        recent_deployments = [
            dep
            for dep in context.deployment_events
        ...
        latency_anomalies = [
            ma
            for ma in context.metric_anomalies
            if "latency" in ma.metric_name.lower()
            or "duration" in ma.metric_name.lower()
        ]
        ...
        if latency_anomalies:
            confidence += 0.3
            evidence.append(
                f"Latency spikes detected after deployment {deployment.version}"
            )
            affected_services.extend(
                [ma.labels.get("service", "unknown") for ma in latency_anomalies]
            )

        if error_anomalies:
            confidence += 0.2
            evidence.append(
                f"Error rate increases detected after deployment {deployment.version}"
            )
            affected_services.extend(
                [ma.labels.get("service", "unknown") for ma in error_anomalies]
            )
        ...
        if deployment_related_errors:
            ...
            evidence.extend(
                [
                    f"Deployment-related errors: {ls.pattern}"
                    for ls in deployment_related_errors
                ]
            )
        ...
        if confidence > 0.6:
            cause = RootCause(
                ...
                related_metrics=[
                    ma.metric_name for ma in latency_anomalies + error_anomalies
                ],
    ...
    def _analyze_database_issues(self, context: IncidentContext) -> list[RootCause]:
        ...
        db_metrics = [
            ...
            for ma in context.metric_anomalies
            if any(
                keyword in ma.metric_name.lower()
                for keyword in ["database", "db", "sql", "connection", "query"]
            )
        ]
        ...
        slow_db_traces = [
            to
            for to in context.trace_outliers
            if any(
                keyword in to.operation_name.lower()
                for keyword in [
                    "database",
                    "sql",
                    "query",
        ...
        if db_metrics:
            confidence += 0.3
            evidence.extend(
                [f"Database metric anomaly: {ma.description}" for ma in db_metrics]
            )
            affected_services.extend(
                [ma.labels.get("service", "unknown") for ma in db_metrics]
            )
        ...
        if slow_db_traces:
            ...
            evidence.extend(
                [
                    f"Slow database operation: {to.operation_name} ({to.duration_ms:.0f}ms)"
                    for to in slow_db_traces
                ]
            )
        ...
        if confidence > 0.6:
            cause = RootCause(
                ...
                timeframe=(
                    context.timestamp - timedelta(minutes=10),
                    context.timestamp,
                ),
                remediation_suggestions=[
    ...
    def _analyze_infrastructure_degradation(
        self, context: IncidentContext
    ) -> list[RootCause]:
        ...
        infra_events = [
            se
            for se in context.system_events
            if se.event_type
            in ["node_restart", "pod_eviction", "memory_pressure", "disk_full"]
        ]
        ...
        resource_metrics = [
            ...
            if any(
                keyword in ma.metric_name.lower()
                for keyword in ["cpu", "memory", "disk", "network", "load", "usage"]
            )
        ]
        ...
        if resource_metrics:
            ...
            evidence.extend(
                [
                    f"Resource metric anomaly: {ma.description}"
                    for ma in resource_metrics
                ]
            )
        ...
        if infra_errors:
            confidence += 0.1
            evidence.extend(
                [f"Infrastructure error: {ls.pattern}" for ls in infra_errors]
            )
        ...
        if confidence > 0.6:
            cause = RootCause(
                ...
                timeframe=(
                    context.timestamp - timedelta(minutes=15),
                    context.timestamp,
                ),
                remediation_suggestions=[
    ...
    def _analyze_dependency_failures(self, context: IncidentContext) -> list[RootCause]:
        ...
        if dependency_errors:
            confidence += 0.3
            evidence.extend(
                [f"Dependency error: {ls.pattern}" for ls in dependency_errors]
            )
        ...
        if dependency_traces:
            confidence += 0.2
            evidence.extend(
                [f"Slow external call: {to.operation_name}" for to in dependency_traces]
            )
        ...
        http_5xx_errors = [
            ls
            for ls in dependency_errors
            if any(code in ls.pattern for code in ["502", "503", "504"])
        ]
        if http_5xx_errors:
            confidence += 0.2
    ...
    def _analyze_resource_exhaustion(self, context: IncidentContext) -> list[RootCause]:
        ...
        resource_metrics = [
            ...
            if any(
                keyword in ma.metric_name.lower()
                for keyword in ["memory", "heap", "gc", "cpu", "thread", "connection"]
            )
        ]
        ...
        if resource_errors:
            confidence += 0.4
            evidence.extend(
                [f"Resource exhaustion error: {ls.pattern}" for ls in resource_errors]
            )
        ...
        if resource_metrics:
            ...
            affected_services.extend(
                [ma.labels.get("service", "unknown") for ma in resource_metrics]
            )
    ...
    def _analyze_configuration_errors(
        self, context: IncidentContext
    ) -> list[RootCause]:
        ...
        if recent_deployments:
            confidence += 0.2
            evidence.append(
                f"Configuration errors following deployment {recent_deployments[0].version}"
            )
    ...
    def _init_deployment_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "metric_patterns": ["latency", "duration", "error_rate", "exception"],
        }
    ...
    def _init_database_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "metric_patterns": ["database", "db", "sql", "connection", "query"],
        }
    ...
    def _init_infrastructure_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "metric_patterns": ["cpu", "memory", "disk", "network", "load", "usage"],
        }
