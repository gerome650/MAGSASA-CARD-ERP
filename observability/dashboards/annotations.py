...
class GrafanaAnnotationService:
    ...
    def _get_dashboards(self) -> list[dict[str, Any]]:
        ...
        try:
            response = self.session.get(
                f"{self.grafana_url}/api/search", params={"type": "dash-db"}, timeout=10
            )
    ...
    def _get_dashboard_details(self, dashboard_id: int) -> dict[str, Any] | None:
        ...
        try:
            response = self.session.get(
                f"{self.grafana_url}/api/dashboards/id/{dashboard_id}", timeout=10
            )
            response.raise_for_status()
    ...
    def find_panel_by_title(
        self, dashboard_title: str, panel_title: str
    ) -> tuple | None:
        """
    ...
    def create_annotation(self, annotation: GrafanaAnnotation) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.grafana_url}/api/annotations", json=payload, timeout=10
            )
            response.raise_for_status()
    ...
    def create_anomaly_annotation(
        self, anomaly_data: dict[str, Any], dashboard_mapping: dict[str, str]
    ) -> bool:
        """
    ...
    def create_alert_annotation(
        self, alert_data: dict[str, Any], dashboard_mapping: dict[str, str]
    ) -> bool:
        """
    ...
    def create_deployment_annotation(
        self, deployment_info: dict[str, Any], dashboard_title: str = "Service Overview"
    ) -> bool:
        """
        ...
        tags = ["deployment", f"version-{version}", "runtime-intelligence"]
...
class AnnotationManager:
    ...
    def handle_anomaly_detection(self, anomaly_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Anomaly annotation created for {anomaly_data.get('metric_name')}"
                )
            else:
                logger.warning(
                    f"Failed to create anomaly annotation for {anomaly_data.get('metric_name')}"
                )
    ...
    def handle_alert(self, alert_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Alert annotation created for {alert_data.get('labels', {}).get('alertname')}"
                )
            else:
                logger.warning(
                    f"Failed to create alert annotation for {alert_data.get('labels', {}).get('alertname')}"
                )
    ...
    def handle_deployment(self, deployment_info: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Deployment annotation created for version {deployment_info.get('version')}"
                )
            else:
                logger.warning("Failed to create deployment annotation")
