            ...
            blocks = [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Service:* {context.service}"
                        },
                        {
                            "type": "mrkdwn",
    ...
    def send_alert(self, alert: Dict[str, Any], context: AlertContext) -> bool:
        ...
        try:
            ...
            if context.severity != AlertSeverity.CRITICAL:
                logger.info(f"Skipping PagerDuty notification for non-critical alert: {context.severity}")
                return True
            ...
            payload = {
                ...
                "payload": {
                    ...
                    "severity": "critical" if context.severity == AlertSeverity.CRITICAL else "error",
                    "component": alert.get('labels', {}).get('service', context.service),
                    "group": context.team,
            ...
            if context.runbook_url:
                links.append({
                    "href": context.runbook_url,
                    "text": "Runbook"
                })
            if context.grafana_url:
                links.append({
                    "href": context.grafana_url,
                    "text": "Dashboard"
                })
...
class SmartAlertRouter:
    ...
    def extract_context(self, alert: Dict[str, Any]) -> AlertContext:
        ...
        baseline_value = annotations.get('baseline_value') or labels.get('baseline_value')
        ...
        if alert.get('startsAt'):
            try:
                timestamp = datetime.fromisoformat(alert['startsAt'].replace('Z', '+00:00'))
            except:
    ...
    def should_suppress_alert(self, context: AlertContext, alert: Dict[str, Any]) -> bool:
        """
        ...
        if context.severity in [AlertSeverity.WARNING, AlertSeverity.INFO]:
            if datetime.now().weekday() >= 5:  # Saturday = 5, Sunday = 6
                return True
        
        # Check for duplicate alerts (simple deduplication)
        alert_key = f"{context.service}_{context.category}_{context.severity.value}"
    ...
    def route_alert(self, alert: Dict[str, Any]) -> Dict[str, bool]:
        ...
        if self.should_suppress_alert(context, alert):
            logger.info(f"Alert suppressed: {context.service} - {context.category}")
