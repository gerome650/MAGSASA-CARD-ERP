...
class EWMAAnomalyDetector:
    ...
    def update(self, value: float) -> AnomalyDetectionResult:
        ...
        if self.ewma_var is None:
            ...
            self.ewma_var = (
                self.alpha * (value - prev_ewma) ** 2 + (1 - self.alpha) * self.ewma_var
            )
...
class RuntimeIntelligenceEngine:
    ...
    def __init__(
        ...
        self.detectors = {
            ...
            "response_time": RollingPercentileDetector(
                window_size=100, percentile=95.0, threshold_multiplier=2.0
            ),
    ...
    def fetch_metric_value(self, query: str) -> float | None:
        """
