...
def generate_loan_status_and_amount(crop_name, crops_data, agscore):
    ...
    if agscore >= 800:
        ...
        loan_status = random.choices(
            ["Approved", "Disbursed", "Repaying", "Pending"], weights=[30, 25, 20, 25]
        )[0]
    elif agscore >= 650:
        ...
        loan_status = random.choices(
            ["Pending", "Rejected", "Under Review"], weights=[30, 50, 20]
        )[0]
...
def generate_realistic_farmer(locations, crops_data):
    ...
    loan_amount, loan_status = generate_loan_status_and_amount(
        primary_crop, crops_data, agscore
    )
...
def generate_farmers(num_farmers=2500):
    ...
    if not locations or not crops_data:
        print(
            "Error: Enhanced data not found. Please run enhance_philippine_data.py first."
        )
        return
