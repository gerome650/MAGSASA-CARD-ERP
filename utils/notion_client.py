...
class NotionClient:
    ...
    def _validate_database_id(self, database_id: str, operation: str = "operation") -> None:
        """
        ...
        """
        if not database_id:
            raise ValueError(f"❌ Database ID is required for {operation}. Please set the appropriate *_DB_ID environment variable.")
    ...
    def query_database(self, database_id: str, filter_params: Optional[Dict] = None, 
                      sort_params: Optional[List[Dict]] = None, page_size: int = 100) -> List[Dict[str, Any]]:
        """
    ...
    def get_database(self, database_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Database retrieved successfully")
        return result
    ...
    def create_database(self, parent_page_id: str, title: str, description: str, 
...
def get_safe_database_id(project: str, database_type: str) -> Optional[str]:
    ...
    if database_id and validate_database_access(database_id, f"{project} {database_type}"):
        return database_id
    return None
...
def extract_rich_text_value(property_data: Dict[str, Any]) -> str:
    ...
    if "rich_text" in property_data and property_data["rich_text"]:
        return "".join([item.get("text", {}).get("content", "") for item in property_data["rich_text"]])
    return ""
